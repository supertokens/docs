...
app.post("/migrate-session", async (req, res) => {
    ...
    if(req.headers.authorization !== undefined){
        ...
        let customUserId = await verifyAccessTokenAndRetrieveUserId(access_token);
...
async function verifyAccessTokenAndRetrieveUserId(access_token: string): Promise<string> {
...
You can follow our [guide](/additional-verification/session-verification/protect-api-routes) on how to do this.
:::
