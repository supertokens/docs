...
```tsx {9,10,11,12,13,14,15,16} check=false reason="pre-built UI globals are provided by the host framework"
...
```tsx {11,12,13,14,15} check=false reason="pre-built UI globals are provided by the host framework"
// this goes in the auth route config of your frontend app (once the pre-built UI script has been loaded)
