...
- WebAuthn is supported by the Node.js and Python backend SDKs; the Go SDK does not support it.
...
```tsx title="/app/auth/auth.component.ts" {42} check=false reason="This example omits surrounding application and SuperTokens configuration."
  ...
  private loadScript(src: string) {
    ...
    script.onload = () => {
      supertokensUIInit({
        ...
        recipeList: [supertokensUIWebAuthn.init(), supertokensUISession.init()],
      });
    };
...
supertokens.init({
  ...
  recipeList: [supertokensSession.init(), supertokensWebAuthn.init()],
});
```
</ContentOption>
</DependentContent>
...
async function signIn() {
  try {
    ...
    } else if (response.status === "INVALID_CREDENTIALS_ERROR" || response.status === "FAILED_TO_AUTHENTICATE_USER") {
      // These errors represent various issues with the authenticator, credential or the flow itself.
      // These should be handled individually by you.
