...
<img class="docs-image-content-width" src="/docs-assets/img/oauth/multiple-frontend-domains-with-a-single-backend.png" alt="Multiple Frontend Domains with a Single Backend"/>
...
For each frontend application, create a separate [**OAuth2 client**](/authentication/unified-login/oauth2-basics#client).
...
<DependentContent group="frontend-prebuilt-ui">
<ContentOption title="Reactjs" value="reactjs">
...
- Set `response_type` to `"code"`. The library uses S256 PKCE for code flow and generates and validates `state` (and `nonce` when using OIDC).
...
- `scope` corresponds directly to the space-separated `scope` value
- Set `responseType` to `"code"`. The library uses S256 PKCE for code flow and generates and validates `state` (and `nonce` when using OIDC).
If you are using a multi-tenant setup, you also need to specify the `tenantId` parameter in the authorization URL.
To do this, set `customQueryParams` to `{ tenant_id: "`&lt;TENANT_ID&gt;`" }`.
</ContentOption>
<ContentOption title="Vue" value="vue">
