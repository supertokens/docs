...
<img src="/docs-assets/img/oauth/multiple-frontend-domains-with-separate-backends.png" alt="Multiple Frontend Domains with separate Backends"/>
...
func introspectAndValidateTenant(ctx context.Context, accessToken, expectedTenant string) error {
  form := url.Values{
    "token": {accessToken},
    "scope": {strings.Join(requiredScopes, " ")},
  }
  request, err := http.NewRequestWithContext(ctx, http.MethodPost, introspectionURL, strings.NewReader(form.Encode()))
  if err != nil {
    return err
  }
  ...
  if !tokenInfo.Active || !tokenInfo.Audience.Contains(expectedAudience) || tokenInfo.TenantID != expectedTenant || tokenInfo.ClientID != oauthConfig.ClientID || tokenInfo.Issuer != expectedIssuer {
...
func login(w http.ResponseWriter, r *http.Request) {
  ...
  tenantID := resolveAllowedTenant(r)
  verifier := oauth2.GenerateVerifier()
  transaction := OAuthTransaction{State: state, Verifier: verifier, TenantID: tenantID}
  if err := oauthTransactions.Put(applicationSessionID(r), transaction); err != nil {
    http.Error(w, "login unavailable", http.StatusInternalServerError)
    return
  }
...
func callback(w http.ResponseWriter, r *http.Request) {
  transaction, ok := oauthTransactions.Consume(applicationSessionID(r))
  ...
  token, err := oauthConfig.Exchange(r.Context(), r.URL.Query().Get("code"), oauth2.VerifierOption(transaction.Verifier))
  if err != nil {
    http.Error(w, "code exchange failed", http.StatusBadRequest)
    return
  }
  if err := introspectAndValidateTenant(r.Context(), token.AccessToken, transaction.TenantID); err != nil {
    http.Error(w, "token validation failed", http.StatusUnauthorized)
    return
  }

  newSessionID, err := rotateApplicationSession(w, r)
  if err != nil {
    http.Error(w, "session creation failed", http.StatusInternalServerError)
    return
  }
  applicationSessionStore.Put(newSessionID, AppSession{TenantID: transaction.TenantID, Token: token})
  http.Redirect(w, r, "/", http.StatusFound)
}
```

The snippet uses `crypto/rand`, `encoding/base64`, and the standard HTTP/JSON packages. Set `introspectionURL` to `<YOUR_API_DOMAIN>/auth/oauth/introspect`, `expectedIssuer` to the exact configured issuer, `expectedAudience` to this resource server, and `requiredScopes` to its scopes. Core 12.1.1 introspection can return `aud` as a JSON string or array; `Audience.UnmarshalJSON` handles both and the callback requires the configured audience before creating a session. The released `tenant_id` authorization parameter selects the tenant, and introspection returns its signed `tId`. Only the rotated opaque session ID reaches the browser.
</ContentOption>
