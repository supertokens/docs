...
For a multitenant setup, read `<TENANT_ID>` from the email verification link's `tenantId` query parameter. For the public tenant, omit the `/<TENANT_ID>` path segment.
