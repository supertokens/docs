...
```tsx {10,11,12} check=false reason="script-tag example relies on globals provided by loaded SuperTokens bundles"
...
```tsx {7} check=false reason="script-tag example relies on globals provided by loaded SuperTokens bundles"
supertokens.init({
  appInfo: {
    apiDomain: "...",
    appName: "...",
  },
  recipeList: [supertokensEmailVerification.init(), supertokensSession.init()],
});
```
