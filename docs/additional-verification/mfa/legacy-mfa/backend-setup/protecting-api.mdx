...
```tsx {8,9,10,11} check=false reason="example uses the application-defined SecondFactorClaim from the preceding setup"
...
```python {15,16,17} check=false reason="route fragment assumes an existing framework application"
