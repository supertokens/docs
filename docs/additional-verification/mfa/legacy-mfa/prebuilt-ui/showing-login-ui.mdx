...
```tsx {22,23,24,25,26,27,28,29} check=false reason="application example imports local modules defined elsewhere"
        ...
        functions: (oI) => ({
          ...
          getGlobalClaimValidators: ({ claimValidatorsAddedByOtherRecipes }) => {
            return [
              ...
              ...claimValidatorsAddedByOtherRecipes.filter((v) => v.id !== MultiFactorAuth.MultiFactorAuthClaim.id),
            ];
          },
        }),
      },
...
```tsx {23,24,25,26,27,28,29,30} check=false reason="application example imports local modules defined elsewhere"
