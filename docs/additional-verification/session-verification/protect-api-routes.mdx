...
description: 'Explore three session verification methods with SuperTokens: middleware, function, and manual JWT verification.'
sidebar:
  order: 1
---
...
:::warning[The downside to using JWT verification manually is that:]
...
:::warning[This method is less secure compared to the first method because it disables key rotation of the access token signing key.]
...
<Steps>
<Step title="Query the JWKS.json endpoint:">
<br />
  ...
  :::note[The above shows an example output which returns two keys.]
...
<Step title="Run the NodeJS script below to convert the above output to a PEM file format.">
<br />
...
<Step title="Tell SuperTokens to always only use the static key when creating a new session.">
...
:::warning[Not applicable. Please use method 1 instead.]
:::
...
Claims like email verification and user roles claims are included in the access token by the backend SDK automatically. You can even [add your own custom claims](/additional-verification/session-verification/claim-validation#using-the-access-token-payload) to the access token payload, and those claims will be in the JWT as expected.

:::note[On claim validation failure, you must send a `403` to the frontend, which causes the frontend SDK (pre-built UI SDK) to recheck the claims added on the frontend and navigate to the right screen.]
:::
...
:::note[You need to check for anti-cross-site request forgery (CSRF) for **non** GET requests when cookie-based authentication is active.]
:::

Two methods exist for configuring [cross-site request forgery (CSRF) protection](/post-authentication/session-management/security#anti-csrf): `VIA_CUSTOM_HEADER` and `VIA_TOKEN`.
