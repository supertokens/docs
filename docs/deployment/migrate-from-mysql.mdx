...
:::warning[Make sure to use the same version as the `supertokens-mysql` instance that you are currently running.]
:::
...
:::info[If you do not have permissions to write to the database filesystem you can use the following python script to export tables one by one:]
<Accordion>
...
def run_mysql_command(query: str):
...
:::info[If you cannot disable the triggers, use the order specified in the next step.]
...
<Tab title="Without specifying column order" value="without-order">
```sql
COPY <table_name> FROM '/pg-data-host/<table_name>.csv'
CSV DELIMITER ',' QUOTE '"' ESCAPE '\' NULL as '\N';
```
</Tab>
<Tab title="With specifying column order" value="with-order">
```bash
#!/bin/bash
...
<Steps>
<Step title="Create a staging table">
```sql
...
<Step title="Import data into the staging table:">
```sql
COPY tenant_thirdparty_provider_clients_raw FROM '/host/tenant_thirdparty_provider_clients.txt'
CSV DELIMITER ',' QUOTE '"' ESCAPE '\' NULL as '\N';
```
</Step>
<Step title="Convert and insert into the final table:">
```sql
...
<Step title="Import the hexadecimal data">
```sql
