...
## Select immutable deployment artifacts

This guide does not currently define an official compatibility mapping between a Core image, PostgreSQL image, database
plugin, binary, and Helm chart. Do not infer one by using an untagged image or `latest`, and do not use the deprecated
`supertokens-docker-postgresql` repository as a release contract.

Before production deployment, obtain a supported version tuple from the maintained release channel. Pin container images
by digest (for example, `repository:version@sha256:<digest>`), verify the digest and provenance in CI, scan the images, and
promote the same digests between environments. Keep PostgreSQL and Core on a private network; expose only your trusted
backend or TLS edge. Until that mapping is published and tested, the installation examples on this page are not a
reproducible production deployment contract.
