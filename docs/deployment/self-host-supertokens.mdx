...
description: Deploy SuperTokens Core with Docker or on a VM, connect PostgreSQL 13 or newer for persistent storage, and verify service health.
sidebar:
  order: 1
---
...
- Persistent deployments support PostgreSQL 13.0 or newer. Core 11.0.0 dropped MySQL and MongoDB support; an in-memory database is available for testing.
- Core listens on port 3567 by default. The `/hello` endpoint returns success only when Core and its database connection are healthy.
