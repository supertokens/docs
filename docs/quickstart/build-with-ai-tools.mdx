...
<Prompt
  description="Use the SuperTokens integration skill to implement authentication."
  actions={["copy"]}
>
Use the `supertokens-integration` skill for this task. Inspect the repository first, identify the application stack and existing authentication setup, and ask for any missing product decisions. Use the current SuperTokens documentation and SDK APIs, implement the integration without committing secrets, and run the relevant tests, typechecks, and build before reporting the result.
</Prompt>
...
For a complete integration task, ask the agent to use the `supertokens-integration` skill. It guides the agent through inspecting your project, selecting the right recipes, configuring the frontend and backend together, protecting secrets, and validating the result.
