...
:::warning[Don't use `getSession` or `verifySession` here. They might update the session tokens, and since our request interceptors don't run server-side, the updated token won't be propagated to the client side.]
:::
