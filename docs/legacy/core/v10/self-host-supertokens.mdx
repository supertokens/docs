...
# Self-host a legacy SuperTokens Core version
...
```bash
docker run -p 3567:3567 -d supertokens/supertokens-postgresql^{docker_version_postgresql}
```
...
services:
  ...
  supertokens:
    image: supertokens/supertokens-postgresql^{docker_version_postgresql}
    depends_on:
      db:
        condition: service_healthy
    ports:
