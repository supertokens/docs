...
description: Learn how to prevent duplicate user accounts through account deduplication techniques.
sidebar:
  order: 60
---
...
Finally, the `signInUpPOST` API is overridden to catch that custom error and return a [general error status](/references/backend-sdks/api-overrides#error-management) to the frontend with a message displayed to the user in the sign in form.
