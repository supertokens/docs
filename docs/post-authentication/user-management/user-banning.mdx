...
Besides the new actual plugin initialization, you also have to include the `UserRoles` recipe in your SuperTokens configuration.
...
const banResponse = await fetch("/plugin/supertokens-plugin-user-banning/ban?tenantId=public", {
  ...
  body: JSON.stringify({
    ...
    // You can also pas the userId instead of the email
    // userId: "user123", 
...
To create a custom user interface you can make use of the `usePluginContext` hook.
...
<ReferenceCard.Grid>
  ...
  <ReferenceCard href="/docs/post-authentication/user-management/user-roles">
    <ReferenceCard.Title>
      User Roles
    </ReferenceCard.Title>
    <ReferenceCard.Description>
