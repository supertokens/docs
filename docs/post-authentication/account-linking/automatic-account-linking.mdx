...
:::note
If you are returning `shouldRequireVerification` as `true`, then you need to also [enable the email verification recipe](/additional-verification/email-verification/initial-setup) in `REQUIRED` mode. This means that if the login method does not inherently verify the email (like for email password login), SuperTokens requires the user to go through the email verification flow first. Then, it attempts auto linking of the account. For other login methods like sign in with Google, the email is already verified during login. The user does not need to verify the email again, and account linking occurs immediately.
...
:::warning[If your logic in `onAccountLinked` throws an error, then it is not called again, and still results in linking the accounts. However, the end user would see an error on the UI as the API returns a `500` status code. They can retry the login action and log into the primary user's account as expected.]
:::
...
<Accordion>
<AccordionItem title="ERRCODE001 ">
- This can happen during creating a password reset code in the email password flow:
    ...
    <img src="/docs/img/account-linking/err_001.png" alt="pre-built UI screenshot showing error message for ERR_CODE_001." style={{ height: "400px" }}/>
...
<AccordionItem title="ERRCODE002">
- This can happen during the passwordless recipe's create or consume code API (during sign up):
    ...
    <img src="/docs/img/account-linking/err_002.png" alt="pre-built UI screenshot showing error message for ERR_CODE_002." style={{ height: "400px" }} />
...
- This can happen during the third party recipe's `/signinup` API (during sign in):
    ...
    ```
    - The pre-built UI on the frontend displays this error in the following way:
    <img src="/docs/img/account-linking/err_004.png" alt="Pre-built UI screenshot showing error for message ERR_CODE_004." style={{ height: "400px" }}/>
...
<AccordionItem title="ERRCODE005">
- This can happen during the third party recipe's `signinup` API (during sign in):
    - API path and method: `/signinup POST`
    - Output JSON:
    ```json
    ...
    ```
    - The pre-built UI on the frontend displays this error in the following way:
    <img src="/docs/img/account-linking/err_005.png" alt="Pre-built UI screenshot showing error message for ERR_CODE_005." style={{ height: "400px" }}/>
...
<AccordionItem title="ERRCODE006">
- This can happen during the third party recipe's `signinup` API (during sign up):
    ...
    ```
    - The pre-built UI on the frontend displays this error in the following way:
    <img src="/docs/img/account-linking/err_006.png" alt="Pre-built UI screenshot showing error message for ERR_CODE_006." style={{ height: "400px" }}/>
...
<AccordionItem title="ERRCODE007">
- This can happen during the email password sign up API:
    ...
    ```
    - The pre-built UI on the frontend displays this error in the following way:
    <img src="/docs/img/account-linking/err_007.png" alt="Pre-built UI screenshot showing error message for ERR_CODE_007." style={{ height: "400px" }}/>
...
<AccordionItem title="ERRCODE008">
- This can happen during the email password sign in API:
    ...
    ```
    - The pre-built UI on the frontend displays this error in the following way:
    <img src="/docs/img/account-linking/err_008.png" alt="Pre-built UI screenshot showing error message for ERR_CODE_008." style={{ height: "400px" }}/>
...
<AccordionItem title="ERRCODE020">
...
<AccordionItem title="ERRCODE021">
- This can happen during association of a third party login to an existing session's account.
    - API Path is `/signinup POST`.
    - Output JSON:
        ```json
...
<AccordionItem title="ERRCODE022">
- This can happen during association of a third party login to an existing session's account.
    - API Path is `/signinup POST`.
    - Output JSON:
        ```json
...
<AccordionItem title="ERRCODE023">
- This can happen during association of a third party login to an existing session's account.
    - API Path is `/signinup POST`.
    - Output JSON:
        ```json
...
<AccordionItem title="ERRCODE024">
