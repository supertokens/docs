...
```tsx {11} check=false reason="Requires SDK globals from surrounding application"
...
```tsx {8} check=false reason="Requires SDK globals from surrounding application"
supertokens.init({
  appInfo: {
    apiDomain: "...",
    appName: "...",
...
```python {16} check=false reason="Partial configuration example"
