...
```tsx {2} check=false reason="Requires SDK globals from surrounding application"
async function logout() {
...
```python {5,6,7,8,9,10} check=false reason="Requires surrounding framework application context"
...
```python {9,10,11} check=false reason="Requires surrounding async application context"
