...
### Core and database config
There are two types of configurations:
- Core: These key-values define the http service's and session's behaviour. These are explained in the section below
- Database: These key-values are specific to your chosen database. Please visit the [Database configuration page](./database/mysql) to learn about these.
...
There may be more compulsory config values to set for your database. Please visit the database config page
</div>
...
- **cookie_secure**:
    ...
    - Default: If running in DEV mode (using ```supertokens start dev```), this value by default is ```false```. In PRODUCTION mode (using ```supertokens start production```), the default value is ```true```
