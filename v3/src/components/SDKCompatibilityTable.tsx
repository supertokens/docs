...
import { getCompatibility, getSupportedDrivers, getSupportedFrontends } from "../lib/api";
import { Text, ScrollArea, Skeleton, Badge, Card, Flex, Separator, Grid, Select, Heading, Box } from "@radix-ui/themes";
...
export function SDKCompatibilityTable() {
  ...
  const [selectedBackendSDKVersion, setSelectedBackendSDKVersion] = useState<string>("");
  const {
  ...
  return (
    <>
        ...
        <Card>
          <Form.Root asChild>
            <Flex gap="2" direction="column">
              ...
              <Text color="gray">Select the SDKs that you are using in order to load the available versions.</Text>
                      ...
                      >
                        ...
                        <Select.Content>
                          {supportedBackends?.drivers?.map((option) => (
                            <Select.Item key={`selected-backend-${option.id}`} value={option.id}>
                              {option.displayName}
                            </Select.Item>
                          ))}
                        </Select.Content>
                      ...
                      >
                        ...
                        <Select.Content>
                          {supportedFrontends?.frontends?.map((option) => (
                            <Select.Item key={`selected-frontend-${option.id}`} value={option.id}>
                              {option.displayName}
                            </Select.Item>
                          ))}
                        </Select.Content>
      ...
      >
        <Box px="5" py="5" asChild>
          <Form.Root asChild>
            <ScrollArea type="auto" scrollbars="vertical" style={{ height: 500 }} asChild>
              <Card>
                <Flex gap="2" direction="column" style={{ height: "100%" }}>
                        ...
                        >
                          ...
                          <Select.Content>
                            {coreVersions?.map((option) => (
                              <Select.Item key={`core-option-${option.id}`} value={option.id}>
                                {option.displayName}
                              </Select.Item>
                            ))}
                          </Select.Content>
                  ...
                  {supportedBackendVersions && supportedBackendVersions.length > 0 ? (
                    <Flex gap="2" direction="row" wrap="wrap">
                      {supportedBackendVersions.map((version) => (
                        <Badge key={`backend-version-${version}`} color="gray" variant="outline" size="3">
                          {version}
                        </Badge>
                      ))}
                    </Flex>
          ...
          <Form.Root asChild>
            <ScrollArea type="auto" scrollbars="vertical" style={{ height: 500 }} asChild>
              <Card style={{ height: "500px" }}>
                <Flex gap="2" direction="column" wrap="wrap" style={{ height: "100%" }}>
                  <Box mb="1" asChild>
                    <Heading as="h3" size="3" color="orange">
                        ...
                        >
                          ...
                          <Select.Content>
                            {backendVersions?.map((option) => (
                              <Select.Item key={`backend-version-option-${option.id}`} value={option.id}>
                                {option.displayName}
                              </Select.Item>
                            ))}
                          </Select.Content>
                  ...
                  <Separator size="4" mt="2" mb="3" />
                  {supportedFrontendVersions && supportedFrontendVersions.length > 0 ? (
                    <Flex gap="2" direction="row" wrap="wrap">
                      {supportedFrontendVersions.map((version) => (
                        <Badge key={`frontend-version-${version}`} color="gray" variant="outline" size="3">
                          {version}
                        </Badge>
                      ))}
                    </Flex>
...
function useFetchSupportedFrontends() {
  const [data, setData] = useState<Awaited<ReturnType<typeof getSupportedFrontends>> | null>(null);
...
function useFetchSupportedBackends() {
  const [data, setData] = useState<Awaited<ReturnType<typeof getSupportedDrivers>> | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<any>(null);
...
function useFetchSdkCompatibility(selectedFrontend: string, selectedBackend: string) {
  const [data, setData] = useState<Awaited<ReturnType<typeof getCompatibility>> | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await getCompatibility(selectedBackend, selectedFrontend);
        setData(response);
      } catch (err) {
        setError(err);
      } finally {
