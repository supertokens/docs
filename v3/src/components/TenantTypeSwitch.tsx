		...
		<Flex gap="2" direction="column" mb="6" py="5" px="4" asChild>
			<Card>
				...
				<Flex direction="row" gap="2" align="start">
					<RadioCards.Root
						defaultValue={tenantType === "multi" ? "yes" : "no"}
						columns={{ initial: "1", sm: "2" }}
					>
						<RadioCards.Item
							value="yes"
							onClick={() => onChangeTenantType("multi")}
						>
						...
						<RadioCards.Item
							value="no"
							onClick={() => onChangeTenantType("single")}
						>
							<Flex direction="column" width="100%" height="100%" align="start">
...
function SingleTenantContent({ children }: React.PropsWithChildren<{}>) {
	...
	return (
		<HeadingFilter name="single">
			{state.tenantType === "single" ? children : null}
		</HeadingFilter>
	);
}
...
function MultiTenantContent({ children }: React.PropsWithChildren<{}>) {
	...
	return (
		<HeadingFilter name="multi">
			{state.tenantType === "multi" ? children : null}
		</HeadingFilter>
	);
}
