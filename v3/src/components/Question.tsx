import React, { createContext, Children, PropsWithChildren, useState, useContext, useCallback, useEffect } from "react";
...
export function Question(
  ...
  const [selectedAnswer, setSelectedAnswer] = useState<string | undefined>(defaultAnswer);
  ...
  useEffect(() => {
    const storedAnswer = localStorage.getItem(`${LOCAL_STORAGE_KEY_PREFIX}:${question}`);
      ...
      <Flex gap="2" direction="column" mb={mb} py={py} px={px} width="fit-content" asChild>
        <Card variant={variant}>
          ...
          <Flex direction="row" gap="2" align="start">
            <RadioCards.Root defaultValue={defaultAnswer} columns={{ initial: "1", sm: "2" }}>
              {children}
            </RadioCards.Root>
          </Flex>
        </Card>
...
export function Answer(props: PropsWithChildren<AnswerProps>) {
  ...
  return (
    <RadioCards.Item value={props.title} onClick={onClick} className="radio-card-item">
