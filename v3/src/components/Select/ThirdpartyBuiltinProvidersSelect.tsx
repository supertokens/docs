...
export function useThirdPartyBuiltinProvidersSelection() {
	return useSelectionStore(
		ThirdPartyBuiltinProvidersSelectionName,
		ThirdPartyBuiltinProvidersDefaultValue,
	);
}

export function ThirdPartyBuiltinProvidersSelect({
	exclude,
}: {
	exclude?: string[];
}) {
	...
	const selectOptions = useMemo(() => {
		if (exclude)
			return ThirdPartyBuiltinProviderOptions.filter(
		...
		<Select.Root value={value} onValueChange={setValue}>
			...
			<Select.Content>
				{selectOptions.map((option) => (
					<Select.Item
						key={`${ThirdPartyBuiltinProvidersSelectionName}-${option.value}`}
						value={option.value}
					>
						{option.label}
					</Select.Item>
