...
export function useNodeFrameworksSelection() {
  return useSelectionStore(NodeFrameworksSelectionName, NodeFrameworksDefaultValue);
}

export function NodeFrameworksSelect({ exclude }: { exclude?: string[] }) {
  const [value, setValue] = useNodeFrameworksSelection();
  const selectOptions = useMemo(() => {
    if (exclude) return NodeFrameworksOptions.filter((tabItem) => !exclude.includes(tabItem.value));
    ...
    <Select.Root value={value} onValueChange={setValue}>
      ...
      <Select.Content>
        {selectOptions.map((option) => (
          <Select.Item key={`${NodeFrameworksSelectionName}-${option.value}`} value={option.value}>
            {option.label}
          </Select.Item>
        ))}
      </Select.Content>
