...
import "./AppInfoForm.scss";

export function AppInfoForm() {
  const { appInfo, onChangeAppInfoField } = useContext(DocItemContext);

  const [formErrors, setFormErrors] = useState<Record<keyof typeof appInfo, string>>({
    appName: "",
    apiBasePath: "",
    apiDomain: "",
    websiteDomain: "",
    ...
    <Card style={{ padding: "0" }} mb="4">
      <Form.Root asChild>
        <Flex gap="2" direction="column" py="5" px="4">
          ...
          <Text>
            Adjust these values based on the application that you are trying to configure. To learn more about what each
            field means check the <a href="/docs/thirdpartyemailpassword/appinfo">references page</a>.
          </Text>
          <Grid
            columns={{
          ...
          >
            ...
            <Form.Field name="websiteBasePath" asChild>
              <Flex gap="1" direction="column">
                ...
                <VisuallyHidden>
                  <Text as="span">The path where the login UI will be rendered</Text>
                </VisuallyHidden>
                <Form.Control asChild>
                  <TextField.Root
