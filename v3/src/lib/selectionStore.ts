...
/**
 * Data store that is used to syncronize different
...
const getLocalStorageKey = (selectionName: string) =>
	`${LocalStorageKeyPrefix}-${selectionName}`;
const subscribe =
	(selectionName: string, initialValue: unknown, useLocalStorage: boolean) =>
	(callback: () => void) => {
		if (!SelectionStore.has(selectionName)) {
			let value = initialValue;
			if (useLocalStorage) {
				const localStorageValue = localStorage.getItem(
					getLocalStorageKey(selectionName),
				);
...
export function useSelectionStore<T>(
	...
	const set = (value: T) => {
		...
		SelectionStore.get(selectionName).subscribers.forEach((callback) =>
			callback(),
		);
	};
