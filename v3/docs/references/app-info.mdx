...
sidebar_position: 5
description: >-
  Configure `appInfo` for frontend and backend to manage app name, domains, and
  API paths.
---
...
By default, the login UI will be displayed on `{websiteDomain}/auth`. Other authentication-related user interfaces will be shown on `{websiteDomain}/auth/*`.
