...
- We no longer need the custom `secondFactoeClaim` since the MFA recipe adds its [own MFA claim](/docs/additional-verification/mfa/important-concepts#how-are-auth-factors-marked-as-completed).
