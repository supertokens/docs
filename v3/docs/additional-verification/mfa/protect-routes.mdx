...
In this section, we will talk about how to protect your frontend and backend routes to make them accessible only when the user has finished all the MFA challenges configured for them.
...
:::important
This validator is added globally, which means that every time you use `verifySession` or `getSession` from our backend SDKs, this check will happen. This means that you don't need to add any extra code on a per API level to enforce MFA.
:::
...
:::important
Make sure to also do other checks on the JWT's payload. For example, if you require all users to have finished email verification, then we need to check for that claim as well in the JWT.
:::
