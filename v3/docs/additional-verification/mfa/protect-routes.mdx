...
In both the backend and the frontend, routes are protected based on value of [the MFA claim](./important-concepts#how-are-auth-factors-marked-as-completed), in the session's access token payload.
...
This validator checks that the value of `v` in the [MFA claim](./important-concepts#how-are-auth-factors-marked-as-completed) is `true` before allowing the request to proceed.
