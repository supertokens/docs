...
Start by following the recipe guide for first factor login. To continue building our example app, we will use the [thirdparty + emailpassword recipe](https://supertokens.com/docs/thirdpartyemailpassword/introduction) as the first factor.
...
From here on, you can continue to build out the first factor's login form using the functions exposed from the `supertokens-web-js` SDK. See the [thirdparty + emailpassword recipe guide](https://supertokens.com/docs/thirdpartyemailpassword/introduction) for more information about the flows and functions.
