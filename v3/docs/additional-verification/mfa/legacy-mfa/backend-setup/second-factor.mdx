...
We also have to be careful about not creating a new session after the second factor auth is completed. By default, the passwordless recipe will create a new session on successful verification, overwriting the older one. We can prevent this, by using the `userContext` feature:
