...
Step-up authentication can be used with the following factors:
- `TOTP`
...
If they haven't, you should return a `403` to the frontend which highlights which factor is required.
...
- We then further check that if the user has finished the TOTP login method within the last 5 minutes. If they haven't, we send back a 403 to the frontend for the frontend to handle.
...
By default, SuperTokens allows a factor setup (for example, creating a new TOTP device), as long as the user has a session and has completed all the MFA factors required during login.
...
In order to prevent this, we need to override one fo the MFA recipe functions on the backend to enforce that the factor setup can only happen if the user is not in a step up auth state:
...
- Then we check if the user has TOTP already setup for them, if they haven't, then we allow the factor setup (otherwise the user would not be able to complete the step up auth challenge). If they have, we do the same check we did in step 1 - checking if the user has finished TOTP in the last 5 minutes or not. If they haven't, we disallow factor setup.
...
This is a different scenario that the above steps cause here, you do not want to reply on an API call to fail, instead you want to check for the step up auth condition before rendering the page itself. 

To do this, we read the access token payload on the frontend and check the completed time of the factor we care about before rendering the page.
If the completed time is older than 5 minutes (as an example), we should redirect the user to the factor challenge page.
...
```
- We check if the user has completed TOTP within the last 5 minutes or not. If not, we show a message to the user, and ask them to complete TOTP.
...
- For checking for step up auth, we get the MFA claim value from the session and then check if TOTP was completed within the last 5 minutes. Only if it was, we return true, else we return false.
- Notice that we use a `DateProviderReference` class exported by SuperTokens instead of just doing `Date.now()`. This is done to take into account any clock skew that may exist between the frontend and the backend server.
