...
This will ensure that your component renders only if the user is logged in.
If they are not logged in, the user will be redirected to the login page.
...
In the `AdminRoute` component, we use the `SessionAuth` wrapper to ensure that the session exists.
We also add the `UserRoleClaim` validator to the `<SessionAuth>` component which checks if the validators pass or not.
If all validation passes, we render the `props.children` component.
If the claim validation has failed, it will display the `AccessDeniedScreen` component instead of rendering the children.
...
This may include a validator that enforces that the user's email is verified (if enabled by you).
- We can also add a `PermissionClaim` validator to enforce a permission.
