...
- See the [docs on how to fetch the access token on the frontend for all frameworks](/docs/post-authentication/session-management/access-session-data/access-the-jwt#on-the-frontend) if needed.
