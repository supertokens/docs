...
Socket connections do not always use HTTP, and so we cannot utilise cookies / HTTP authorization header here.
Instead, we must fetch the JWT on the frontend and send that at the start of the socket connection.
...
We need to make sure that we expose the JWT to the frontend.
