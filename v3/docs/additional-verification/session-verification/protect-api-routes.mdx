...
Claims like email verification and user roles claims are included in the access token by the backend SDK automatically. You can even [add your own custom claims](/docs/post-authentication/session-management/claims/access-token-payload) to the access token payload, and those claims will be in the JWT as expected.
