...
Actions like, listing roles, creating permissions or checking which roles have been assigned are available through different SDK calls.

## Before you start

:::info 

Most of the actions outlined on this page can also be done from the user management dashboard.
To know more about how to use it check [the documentation](/docs/post-authentication/dashboard/managing-user-roles-and-permissions)
...
The SDK function only adds missing permissions and will not have any effect on permissions that are already assigned to role.
...
To remove one or more permissions from a role, the role must be created before you use this function.
...
Get a list of all the roles that have been assigned a specific permission.
