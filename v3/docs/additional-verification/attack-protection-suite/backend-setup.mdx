...
hide_title: true
description: >-
  Configure backend for anomaly detection service using regional endpoints and
...
In order to use the anomaly detection service, a request to the following endpoint (based on the region) needs to be made:
...
### Email and password
<BackendTabs additionalValues={[{ label: "Other Frameworks", value: "otherFrameworks" }]}>
...
<BackendTabs.TabItem value="otherFrameworks">

:::note
For using with other frameworks, you can follow the same steps as for the other languages and you can also refer to the [API documentation](#setting-up).
:::
...
It's important to note that anomaly detection is not recommended for use with third-party providers. Several reasons exist for this:
