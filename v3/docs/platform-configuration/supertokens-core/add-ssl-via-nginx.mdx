...
We are going to be using a self signed certificates since we are developing locally, but, you can also use a certificate authorities like [Let's Encrypt](https://letsencrypt.org/) to generate valid certificates.
