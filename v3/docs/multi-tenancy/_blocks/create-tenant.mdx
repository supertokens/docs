...
If `firstFactors` is not specified, by default, none of the login methods are enabled.
...
The request includes the `appId` for which we want to create a new tenant.
