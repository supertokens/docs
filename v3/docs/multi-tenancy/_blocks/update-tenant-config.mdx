...
In the above example, different values are assigned for certain configurations for `customer1` tenant.
...
If not provided, the tenant's information is stored in the database as specified in the core's configuration.
