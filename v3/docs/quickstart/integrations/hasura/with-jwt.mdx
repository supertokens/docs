...
description: >-
  Authenticate API calls using JWTs with SuperTokens and Hasura, including
  custom claims setup.
---
...
Please check our separate [page](/docs/authentication/unified-login/customizations/add-custom-claims-in-tokens) that shows you how to do this.
...
Follow the [quickstart guides](/docs/quickstart/frontend-setup) to add **SuperTokens**.
...
You can get the JWKS URL for your backend by using the method explained [here](/docs/additional-verification/session-verification/protect-api-routes/manual-jwt-verification#verifying-a-jwt-using-a-jwt-verification-library)

### Using a key string

Hasura let's you provide a PEM string in the configuration. Refer to [this page](/docs/additional-verification/session-verification/protect-api-routes/manual-jwt-verification#method-2-using-public-key-string) to know how to get a public key as a string, you can then use that key string in the Hasura config:
