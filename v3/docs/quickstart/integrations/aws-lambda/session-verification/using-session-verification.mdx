...
hide_title: true
sidebar_position: 1
description: Verify user sessions in your API using SuperTokens' `verifySession` function.
---
...
Create a `/user` resource and then `GET` method in your API Gateway. Configure the lambda integration and CORS just like we did [for the auth routes](/docs/quickstart/integrations/aws-lambda/backend-config/setup-api-gateway#3-attach-lambda-to-the-any-method-of-the-proxy-resource).
