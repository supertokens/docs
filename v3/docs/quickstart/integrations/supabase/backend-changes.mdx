...
sidebar_position: 3
description: >-
  Override SuperTokens backend to create and use Supabase JWT for database
  access.
---
...
## Step 1. Setup your Next.js app with SuperTokens using with the [Next.js setup guide](/docs/quickstart/integrations/nextjs/app-directory/about)

## Step 2. Store Supabase's configuration values in your `.env` file

You can retrieve Supabase's configuration values from their dashboard and append it to your `.env` file:
...
- In our Next.js app when a user signs up, we want to store the user's email in Supabase. We would then retrieve this email from Supabase and display it on our frontend.
