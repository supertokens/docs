...
Follow the [frontend and backend setup guides](/docs/quickstart/frontend-setup) to setup SuperTokens.
...
declare let withSession: (contextValue: any, func: (sessoin: any) => Promise<any>) => Promise<any>; // typecheck-only, removed from output
declare let typeDefs: any; // typecheck-only, removed from output
