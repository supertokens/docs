...
The password reset feature consists of two different forms:
- One that allows users to request a reset password link over email.
- The other allows them to change their password once they have opened the reset link.

The feature is enabled by default through our pre-built UI components. 
...
You will see this if you navigate to `^{appInfo.websiteBasePath}/reset-password`.
...
You will see this if you navigate to `/auth/reset-password?token=TOKEN`.
...
- One where the user will request a password reset link. 
- Another one where the user will change their password.
...
To customize the template check the [email delivery](/docs/platform-configuration/email-delivery/about) section for more information.
