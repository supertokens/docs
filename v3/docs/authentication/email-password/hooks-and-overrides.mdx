...
Explore the references section for a more in depth guide on [hooks](/docs/references/sdks/frontend-hooks/handle-event) and [overrides](/docs/references/sdks/functions-overrides/about).
...
Overriding the `signIn` function will allow you to introduce your own logic for the sign in process.
Use it to persist different types of data or to trigger various actions.
...
Overriding the `signUp` function will allow you to introduce your own logic for the sign in process.
...
:::caution Not applicable
This section is not applicable for custom UI since you will be calling the sign in API yourself anyway. Anything that you want to do during the password reset flow, can be done based on the result of the API call.
:::
...
Overriding the `passwordResetPOST` function will allow you to introduce your own logic for the password reset process.
