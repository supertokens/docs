...
<img alt="Pre-built sign in form UI with default values for fields" src="/img/emailpassword/signin-with-default-values.png" />
...
When you try to submit login form without filling in required / non-optional fields, the SDK will, by default, show an error stating that the `Field is not optional`. You can customize this error message with `nonOptionalErrorMsg` property in the `formFields` config.
