...
- Only the `supertokens-node` backend SDK supports it as of yet. So if you are using our `golang` or `python` SDK, you will have to wait for the next releases or configure a separate NodeJS **Authorization Service**.
