...
description: >-
  Verify OAuth2 tokens using JWT libraries or introspection endpoint for secure
  validation.
---
...
For information on how to verify **SuperTokens Session Tokens** please refer to the [following section](/docs/additional-verification/session-verification/protect-api-routes/overview).
...
The token will still be valid until its expiration time.
...
def validate_token(token: str) -> bool:
   response = oauth2provider.validate_oauth2_access_token(token, { client_id: "<CLIENT_ID>", scopes: ["<REQUIRED_SCOPE>"] }, True) 
