...
If you have skipped this step, please check our [guides](/docs/guides) based on the authentication methods that you want to use.
...
- The login page is mapped to `^{form_websiteDomain}^{form_websiteBasePath}` (this is also the place where a user ends up after logout)
- The token refresh page is mapped to `^{form_websiteDomain}^{form_websiteBasePath}/try-refresh`
- The logout page is mapped to `^{form_websiteDomain}^{form_websiteBasePath}/logout`
...
For mobile apps you will have to reuse the web authentication flow. Check this [guide](/docs/unified-login/reuse-website-login) for more information.
