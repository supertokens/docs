...
For each of your **`frontend`** applications create a separate [**OAuth2 client**](/docs/authentication/unified-login/introduction#client).
...
Setup the Backend API so that it allows requests from all the frontend domains. 
...
Given that the backend that represents the **Authorization Server** also acts as a **Resource Server** you have to account for this in the session verification process.

This is needed because the flow uses two types of tokens:
...
For more information on how to verify the **OAuth2 Access Tokens** please check our [separate guide](/docs/authentication/unified-login/verify-tokens).
