...
:::info
This page makes use of various **OAuth2** terminology. If you need a refresher on what everything means, please check our [separate page](/docs/authentication/unified-login/introduction#oauth2-basics) that explains most of the concepts.
:::
...
To do this you will need to set the `clientCredentialsGrantAccessTokenLifespan` property in the [**Client**](/docs/authentication/unified-login/introduction#client) creation request body.
...
class ClientCredentialsTokenValidator 
{
    ...
    {
        string apiDomain = "^{appInfo.apiDomain}";
        string apiBasePath = "^{appInfo.apiBasePath}";
        string audience = "<AUDIENCE>";
        string requiredScope = "<REQUIRED_SCOPE>";

        HttpClient client = new HttpClient();
        var response = await client.GetStringAsync($"^{appInfo.apiDomain}^{appInfo.apiBasePath}jwt/jwks.json");
