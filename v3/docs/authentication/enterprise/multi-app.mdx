...
You can use this feature to use one SuperTokens core deployment across several independent apps within your company, and / or to create multiple environments (`dev`, `staging`, `prod` etc..) for one app without having to deploy individual SuperTokens core instances.

## 1. Create a new / update an app in the core

:::caution
This is a paid feature, even if creating an additional development environment on our managed service, or if using our `dev license keys` in case of self hosting. The pricing is $50 / month / additional app. Please reach out to use on [support@supertokens.com](mailto:support@supertokens.com) if you have any questions, or if you want to create several `environments` and want a bulk discount.
:::
