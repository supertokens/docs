...
You can also configure a tenant to have different configurations as per the core's `config.yaml` (or docker env) variables. Below is how you can specify the config, when creating or modifying a tenant:
