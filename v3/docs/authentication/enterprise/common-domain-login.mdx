...
```
Above, in `Session.init` on the frontend, the `hasAccessToCurrentDomain` claim validator is added to the global validators. This means, that [whenever a route is protected](/docs/additional-verification/session-verification/protect-frontend-routes), it checks if `hasAccessToCurrentDomain` has passed, and if not, SuperTokens redirects the user to their right sub domain (via the values set in the `AllowedDomainsClaim` session claim).
