...
We need to start by maintaining an allow list of emails.
...
## 2. Check if the input email or phone number is permitted during sign up

After that, override the `createCodePOST` API to check if the input email / phone number is permitted during sign up. If not allowed, send back a user-friendly message to the frontend.
...
`createCodePOST` is invoked when the user enters their email or phone number to login. Override it to check:
- If there is a user with the input email or phone number, it means they are signing in and the operation is allowed.
- Otherwise, check if the input email / phone number is permitted by calling the `isEmailAllowed` / `isPhoneNumberAllowed` function (implemented above). If not allowed, return a message to the frontend.

Emails / phone numbers can be added to the allow list by calling the `addEmailToAllowlist` / `addPhoneNumberToAllowlist` function implemented above.
