...
This may seem like a strange use case of the user metadata recipe we provide, but it works.
...
## 2. Check if the input email or phone number is allowed during sign up

After that, we override the `createCodePOST` API to check if the input email / phone number is allowed during sign up. If not allowed, we send back a user friendly message to the frontend.
