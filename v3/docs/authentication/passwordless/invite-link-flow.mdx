...
hide_title: true
description: Discover how to implement an invite based sign up flow with the passwordless recipe.
sidebar_position: 10
---

import { BackendTabs, ConditionalContent, NextjsRouterTypeSelect, NodeFrameworksCard, PythonFrameworksCard, GoFrameworksCard } from "/src/components";
...
If a user has not been invited before, their sign in attempt fails.

## 1. Add the ability to invite new users

Next, we see how to make the API in which the admin of the app can create new users and invite them:
...
- The code above uses the default magic link path for the invite link (`/auth/verify`). If you are using the pre-built UI, the frontend SDK automatically logs the user in. If you want to show a different UI to the user, then you can use a different path in the link (by modifying the `inviteLink` string) and make your own UI on that path. If you are making your own UI, you can [use the `consumeCode` function provided by the frontend SDK](/docs/authentication/passwordless/initial-setup) to call the passwordless API that verifies the code in the URL and creates the user.
- You can change the lifetime of the magic link, and therefore the invite link, by following [this guide](/docs/authentication/passwordless/customize-the-magic-link#change-the-link-lifetime).

:::info Multi Tenancy
In the above code snippets, the `"public"` `tenantId` is passed when calling the functions - this is the default `tenantId`. If you are using the multi-tenancy feature, you can pass in a different `tenantId` and this ensures that the user with that email is added only to that tenant.

You also need to pass in the `tenantId` to the createMagicLink function which adds the `tenantId` to the generated magic link. The resulting link uses the `websiteDomain` that is configured in the `appInfo` object in `supertokens.init`, but you can change the link's domain to match that of the tenant before sending it.
:::


## 2. Check if a user was already invited 

Start by overriding the `createCodePOST` API to check if the input email / phone number was already invited before. If not, send back a user-friendly message to the frontend. Check if a user was invited before by checking if they already exist in SuperTokens - because users are created in SuperTokens when they complete the invite flow.
...
`createCodePOST` is called when the user enters their email or phone number to log in. Override it to check:
- If a user exists with the input email or phone number, it means they are signing in and so the operation is allowed.
- Otherwise, it means that the user has not been invited to the app and an appropriate message is returned to the frontend.
