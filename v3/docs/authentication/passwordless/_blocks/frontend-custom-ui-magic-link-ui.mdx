...
You configure the contact method on the next page, where we discuss adding the `SDK` to your backend app.
...
After the initial magic link is sent to the user, you may want to display a resend button to them.
...
Since you save the `preAuthSessionId` and `deviceId` after the initial magic link is sent, you can know if the user is on either **step 3.1** or **step 3.2**. Simply check if these tokens are stored on the device.
