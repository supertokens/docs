---
id: sign-in-up-callbacks
title: Post authentication callbacks
hide_title: true
sidebar_position: 5
description: >-
  Implement post authentication callbacks to trigger events like analytics or
  database updates.
---
...
This method allows you to fire events immediately after a successful `signIn` / `signUp`. For example to send analytics events post `signIn` / `signUp`.
...
This section is not applicable for custom UI since you will be calling the `signInUp` API yourself anyway. Anything that you want to do post `signIn` / `signUp`, can be done based on the result of the API call.
:::
...
- Send analytics events about a `signUp`.
