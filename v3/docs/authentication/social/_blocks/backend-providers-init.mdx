...
- Set the authorisation callback URL to `^{appInfo.websiteDomain}^{appInfo.websiteBasePath}/callback/apple`. Note that Apple doesn't allow `localhost` in the URL. If you are in `dev` mode, you can use the `dev` keys provided above.


:::important
You can find the list of built-in providers [in the documentation](/docs/authentication/social/built-in-providers-config). To add a provider that is not listed, you can follow the guide on [setting up custom providers](/docs/authentication/social/custom-providers).
:::
