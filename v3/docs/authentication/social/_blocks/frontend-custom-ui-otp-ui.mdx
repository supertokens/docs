...
The following section will show you what aspects you need to cover in order to implement the UI for a `OTP`, One-Time Password, flow.
...
When the users submits the form you will have to call the following API to create and send them a OTP.
...
After the OTP was sent to the user, you may want to display a resend button to them.
...
Since you save the `preAuthSessionId` and `deviceId` after the initial OTP is sent, you can know if the user is in 3.1 vs 3.2 by simply checking if these tokens are stored on the device.

If they aren't, you should follow 3.1, else follow 3.2.

:::important
You need to clear these tokens if the user navigates away from the 3.2 page, or if you get a `RESTART_FLOW_ERROR` at any point in time from an API call, or if the user has successfully logged in.
:::
...
When the user enters an OTP you will have to call the following API to verify it
