...
This guide shows you how to determine if a new user was created in the next steps.

#### 2.1 Creating and sending the OTP

You have to add a form that asks the user for their email address or phone number.
When the user submits the form you have to call the following API to create and send them an OTP.

:::info

You configure the contact method on the next page, where we talk about adding the `SDK` to your backend app.
...
#### 2.2 Resending an OTP

After the OTP is sent to the user, you may want to display a resend button to them.
