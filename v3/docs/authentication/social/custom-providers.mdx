...
sidebar_position: 4
description: >-
  Add custom authentication providers to SuperTokens using OAuth or OIDC
  endpoints.
---
...
### 1. Via OAuth endpoints

You can define a custom provider in a couple of ways. The simplest method is to provide the config for the `AuthorizationEndpoint`, `TokenEndpoint`, and the mapping for how the user's ID and email from the provider's profile information endpoint. This can be seen below:
...
### 2. Via Open ID connect (OIDC) endpoints
