...
sidebar_position: 8
description: >-
  Implement email allowlist for third-party sign-ups using SuperTokens user
  metadata recipe.
---
...
This section explains how to achieve this flow.
...
:::info Important
Remember to [initialize the user metadata recipe](/docs/post-authentication/user-management/user-metadata/setup) on the backend `recipeList` during `supertokens.init`.
:::

:::info Multi Tenancy
For a multi tenant setup, you can even store an allow list per tenant. This setup allows limiting sign ups for different emails for different tenants. If doing this, pass in the tenantID to the functions above, which can be obtained from the input to the API overrides shown below.
:::

## 2. Verify if the email is permitted

After that, override the `signInUpPOST` API and the `signInUp` recipe function to verify if the input email is permitted during sign up.
If not permitted, send back a user-friendly message to the frontend.
...
The `signInUpPOST` function executes when the user is redirected to the `app` from the third-party provider post login. The API calls the `signInUp` recipe function to check:
- If a user exists with the input email, it means they are signing in, so the operation is allowed.
- Otherwise, verify if the input email is permitted by calling the `isEmailAllowed` function (implemented above). If not permitted, throw an error with a custom message.
- Finally, override the `signInUpPOST` API to catch this custom error and return a message to the frontend for display to the user.

Emails can be added to the allow list by calling the `addEmailToAllowlist` function implemented above.
