...
After that, we override the `signInUpPOST` API and the `signInUp` recipe function to check if the input email is allowed during sign up.
If not allowed, we send back a user friendly message to the frontend.
