...
description: >-
  Implement user migration to SuperTokens without password hashes using
...
<DescriptionText>
  To following guide will show you how to move your users from your current authentication solution to **SuperTokens**
</DescriptionText>
...
New users might get created in that interval through your previous authentication provider. 
Hence, you will also need to create them in **SuperToknes** to keep the data in sync. 
...
### 1. Perform the bulk migration process

#### 1.1 Export the accounts from your current authentication provider
...
#### 1.2 Add the accounts that should be imported
...
#### 1.3 Monitor the progress of the job
...
#### 1.4 Handle staged users that failed to import
...
### 2. Add the lazy migration flow

#### 2.1 Create new users when they try to sign in through SuperTokens

Before you can allow users to authenticate through the **SuperTokens** integration you need to add one more step.
You have to ensure that any user that might have been created after you have exported all your data can will not experience any authentication issues.
...
This can be implemented by overriding either the `signInPOST` API, for the `EmailPassword` recipe, or the `signInUpPOST` API, for `Passwordless` or `ThirdParty`.
...
#### 2.3 Handle the password reset flow
