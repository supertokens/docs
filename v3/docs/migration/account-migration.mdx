...
The following sub steps will go through all the use cases that you need to cover in order to ensure no downtime during the migration process.

#### 1.1 Create new users when they sign up using your previous provider

The `bulk migration` API exposes an endpoint that allows you to directly create new users.
This way you generate new accounts in **SuperTokens**, on the fly, when someone signs up using your old authentication provider.
...
#### 1.2 Prevent sign ups from users who exist in your previous provider
...
#### 1.3 Create new users when they try to sign in through SuperTokens
...
- Determine if the authentication user needs to be migrated or not by checking if it exists in the previous authentication provider and not in **SuperTokens**.
- Validate the input credentials against the external provider
- Map the external `userId` to the **SuperTokens** `userId`. This will allow SuperTokens functions to reference the user with the external `userId`.
...
#### 1.4 Handle the password reset flow
