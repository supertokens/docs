---
id: third-party
title: Managing ThirdParty Providers
hide_title: true
sidebar_position: 3
description: >-
  Configure and manage third-party social, enterprise, custom, and SAML
  providers for tenant authentication.
---

# Managing ThirdParty Providers
...
On continuing, you will be further asked for the SAML configuration. You have an option to either provide SAML XML directly or via the Metadata URL from the Provider. Also, fill in other details such as `Suffix`, `Name`, `Redirect URLs` and click on `Save` to add the SAML provider.
...
If you did not provide the `Boxy API Key`, you will need to add the `Client ID` and `Secret` obtained by calling the Boxy APIs manually. More details are [available here](/docs/authentication/enterprise/saml/with-boxyhq/integration-steps#4-upload-the-base64-xml-string-to-saml-jackson).
