...
If you did not provide the `Boxy API Key`, you will need to add the `Client ID` and `Secret` obtained by calling the Boxy APIs manually. More details are [available here](/docs/authentication/enterprise/saml/integration-steps#4-upload-the-base64-xml-string-to-saml-jackson).
