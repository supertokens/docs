...
sidebar_position: 6
description: >-
  Link social accounts accounts using the account linking feature.
---
...
First, you need to detect which social login methods are already linked to the user. You can do this by inspecting the [user object](/docs/references/user-object) on the backend and checking the `thirdParty.id` property (the values are like `google`, `facebook` etc).
