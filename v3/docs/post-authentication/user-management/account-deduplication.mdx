...
Finally, override the `signInUpPOST` API to catch that custom error and return a [general error status](/docs/references/sdks/apis-overrides/send-custom-response#using-the-general_error-status) to the frontend with a message displayed to the user in the sign in form.
