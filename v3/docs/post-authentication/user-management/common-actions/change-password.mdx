...
sidebar_position: 3
description: >-
  Guide users to implement a backend route for password updates, including
...
- To learn more about how to use the session verification middleware for other frameworks click [here](/docs/additional-verification/session-verification/protect-api-routes#using-verify-session)
...
- You can now use `session` object to retrieve the logged in user's `userId`.
