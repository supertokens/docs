...
description: >-
  Learn how to access session data including JWT tokens, tenant IDs, and user sessions
  across different programming languages and frameworks.
---

import { AppInfoForm } from '/src/components/Forms';
import { FrontendCustomUITabs, FrontendPrebuiltUITabs, BackendTabs, ReactRouterVersionTabs } from "/src/components/Tabs";
import { TokensCallout } from "/src/components";

import { NodeFrameworksCard, NpmOrScriptsCard,GoFrameworksCard, MobileFrameworksCard, PythonSyncAsyncCard, PythonFrameworksCard } from "/src/components/Cards"
...
Given a user ID, we can fetch all sessions that are currently active for that user in the following way:
...
By default, the method will return all the `session handles` for the user across all the tenants.
