...
You can read the tenant ID on the frontend by readin the `tId` claim from the [access token payload](/docs/post-authentication/session-management/claims/access-token-payload#reading-the-payload-on-the-frontend).
