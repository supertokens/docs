...
- The backend configuration steps should only be done if you are using cookie based authentication.
...
For example, if your API endpoints are `{"api.example.com", "api2.example.com", "api3.example.com"}`, the common portion of these endpoints is `".example.com"` (The dot is important). So you would need to set the following:
...
To avoid locking out users with existing sessions (they will get a 500 error when try to refresh their session), set `olderCookieDomain` to match your previous `cookieDomain`.
...
This will allow the frontend SDK to apply interception and automatic refreshing across all your API calls:
