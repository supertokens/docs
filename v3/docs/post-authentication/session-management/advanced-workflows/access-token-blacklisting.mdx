...
import { NodeFrameworksCard, GoFrameworksCard, PythonFrameworksCard } from "/src/components/Cards";
...
Whilst this makes session verifications fast, it also means that if a session is revoked, the user will still be able to use it until the access token expires.

If you want session verifications to fail immediately after the session has revoked, you should force the session to be checked against the database.
...
:::caution
For managed service users, please check [our rate limit policy](/docs/deployment/rate-limits) before implementing this feature. If you suspect that you will breach the free limit you can:
- [Email us](mailto:support@supertokens.com) to increase your rate limit.
