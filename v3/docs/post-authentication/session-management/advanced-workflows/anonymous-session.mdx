...
## Verifying the JWT
:::caution
Please note that you **cannot** use the `verifySession` or `getSession` functions to verify JWTs from anonymous sessions. The `verifySession` and `getSession` check for the presence of certain claims in the JWT (`sessionHandle`, `refreshTokenHash` etc...) that are added by the Session recipe for authenticated users. [Verification for JWTs from anonymous sessions have to be done manually.](/docs/additional-verification/session-verification/protect-api-routes/manual-jwt-verification).
:::

See the [manual JWT verification section](/docs/additional-verification/session-verification/protect-api-routes/manual-jwt-verification) to see how we can verify the JWT and get its payload.
