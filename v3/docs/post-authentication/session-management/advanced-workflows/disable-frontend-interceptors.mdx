...
If you want to also change how session tokens attach to API calls (like sign out), you can return `false` from the function override. Then, attach custom session headers using the [pre-API hook function](/docs/references/sdks/frontend-hooks/pre-api) on the frontend.
