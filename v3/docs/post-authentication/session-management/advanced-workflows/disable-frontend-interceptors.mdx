...
If you want to also change how session tokens are attached to our API calls (like signOut), you can just return `false` from the function override and then attach custom session headers using the [pre API hook function](/docs/references/sdks/frontend-hooks/pre-api) on the frontend.
