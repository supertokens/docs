...
sidebar_position: 3
description: >-
  Customize errors thrown by the Session recipe in your SuperTokens integration.
---
...
The following page shows what errors are thrown by the SuperTokens Session recipe and how you can customize them.

---

## Unauthorised error

The error is generated when a protected backend API is accessed without a session.
The default behaviour is to clear session tokens (if any) and send a 401 to the frontend.
...
The error is generated when a protected backend API is accessed with a session that doesn't pass the claim validators
THe default behaviour is to send a 403 to the frontend with the errors includes in the body.
...
The error is generated when a [session hijacking](https://en.wikipedia.org/wiki/Session_hijacking) attempt has been detected.
...
The default behaviour is to revoke the session and send a `401` to the frontend.
...
This can also be thrown from the session refresh endpoint if multiple access tokens are present in the request cookies.
The default behaviour is to send a `401` to the frontend.
...
The default behaviour is to send a 200 to the frontend.
