...
export function useMobileFrameworksSelection() {
  return useSelectionStore(MobileFrameworksSelectionName, MobileFrameworksDefaultValue);
}

export function MobileFrameworksSelect({ exclude }: { exclude?: string[] }) {
  const [value, setValue] = useMobileFrameworksSelection();
  const selectOptions = useMemo(() => {
    if (exclude) return MobileFrameworksOptions.filter((tabItem) => !exclude.includes(tabItem.value));
    ...
    <Select.Root value={value} onValueChange={setValue}>
      ...
      <Select.Content>
        {selectOptions.map((option) => (
          <Select.Item key={`${MobileFrameworksSelectionName}-${option.value}`} value={option.value}>
            {option.label}
          </Select.Item>
        ))}
      </Select.Content>
