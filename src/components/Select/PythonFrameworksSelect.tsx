...
export function usePythonFrameworksSelection() {
  return useSelectionStore(PythonFrameworksSelectionName, PythonFrameworksDefaultValue);
}
    ...
    <Select.Root value={value} onValueChange={setValue}>
      ...
      <Select.Content>
        {selectOptions.map((option) => (
          <Select.Item key={`${PythonFrameworksSelectionName}-${option.value}`} value={option.value}>
            {option.label}
          </Select.Item>
        ))}
      </Select.Content>
