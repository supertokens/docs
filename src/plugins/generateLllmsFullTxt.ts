...
(async () => {
  const mdxFilePath = "./docs/quickstart/frontend-setup.mdx";
  // const contentDir = path.join(siteDir, "docs");
  const contentDir = path.join("./", "docs");
  ...
  //   imports.push(...(await extractImportStatements(file.content)));
  // }
  //
  // const actualImports = imports.filter((item) => !item.module.includes("_blocks"));
  // const components = actualImports.map((item) => item.name);
  // const uniqueComponents = [...new Set(components)];

  // allFiles.map((file) => parseMdxContent(file.path)).join("\n\n");

  const quickstartGuideFiles = allFiles.filter((file) => file.path.includes("quickstart-guide"));
  const integrationsFiles = allFiles.filter((file) => file.path.includes("integrations"));
  const authenticationFiles = allFiles.filter((file) => file.path.includes("/authentication"));
  const postAuthenticationFiles = allFiles.filter((file) => file.path.includes("post-authentication"));
  const additionalVerificationFiles = allFiles.filter((file) => file.path.includes("additional-verification"));
  const migrationFiles = allFiles.filter((file) => file.path.includes("/migration"));
  const platformConfigurationFiles = allFiles.filter((file) => file.path.includes("platform-configuration"));
  const deploymentFiles = allFiles.filter((file) => file.path.includes("deployment"));

  const filesWithoutReferences = allFiles.filter((file) => !file.path.includes("references"));

  const llmsFullTxt = (
    await Promise.all(
      filesWithoutReferences
        // .filter((file) => file.path.includes("quickstart") && !file.path.includes("integrations"))
        .map((file) => parseMdxContent(file.path)),
    )
  ).join("\n\n");

  fs.writeFileSync("./llms-full.txt", llmsFullTxt);
  // await withMdx(mdxFilePath);
  // console.log(content);
})();
...
//     loadContent: async () => {
//       // Implementation for plugin functionality would go here
//     },
//     postBuild: async ({ outDir }) => {
//       // Implementation for post-build processing would go here
//     },
//   };
// }
