...
export const trackEvent = (event: string, properties: Record<string, any>, useBeacon = false) => {
  if (!amplitudeInitialized) {
    initAmplitude();
  }
  if (useBeacon) {
...
export const trackBeaconEvent = (event: string, properties: Record<string, any>) => {
  if (!amplitudeInitialized) {
    initAmplitude();
  }
  amplitude.setTransport("beacon");
...
export const identifyUser = (properties: Record<string, amplitude.Types.ValidPropertyType>) => {
  if (!amplitudeInitialized) {
    initAmplitude();
  }
