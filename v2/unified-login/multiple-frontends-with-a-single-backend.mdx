...
Given that the backend that represents the **Authorization Service** will also act as a **Resource Server** we will have to account for this in the way we verify the sessions.
...
async function verifySession(req, res, next) {
  ...
  try {
    ...
    if (
      !supertokens.isErrorFromSuperTokens(err) ||
      err.type !== supertokens.ErrorType.TRY_REFRESH_ERROR
    ) {
      next(err);
    }
  }
  ...
  if (jwt === undefined) {
    next(new Error("No JWT found in the request"));
  }

  try {
    const tokenPayload = await validateToken(jwt);
