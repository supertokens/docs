...
export default function FrontendSDKTabs(props: any) {
    return (
        <Tabs
            ...
            {childContainsTabItemWithValue("angular", props.children) ? null : DefaultAngularTabItem()}
            {props.children}
        </Tabs>
    );
}
    ...
    return (
        <TabItem value="angular">
            <div className="admonition admonition-caution alert alert--warning">
                ...
                <div className="admonition-content">
                    If you are using the pre-built UI you can refer to the <b>ReactJS</b> tab since you will be using the <code>supertokens-auth-react</code> SDK.<br/><br/>


                    If you are using custom UI you can refer to the <b>Plain JavaScript</b> section<br/><br/>

                    
                    We also have an <a href="https://github.com/supertokens/supertokens-auth-react/tree/master/examples/with-angular-thirdpartyemailpassword">example app</a> , the example app uses pre-built UI with email-password and social login but the same setup applies to other recipes as well.
                </div>
            </div>
        </TabItem>
    );
