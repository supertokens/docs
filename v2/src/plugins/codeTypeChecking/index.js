...
async function addCodeSnippetToEnv(mdFile, isSwiftEnabled) {
  if (
    mdFile.includes("/v2/change_me/") ||
    mdFile.includes("/v2/contribute/") ||
    mdFile.includes("/v2/nodejs") ||
    mdFile.includes("/v2/golang") ||
    mdFile.includes("/v2/python") ||
    mdFile.includes("/v2/auth-react") ||
    mdFile.includes("/v2/website") ||
    mdFile.includes("/v2/react-native") ||
    mdFile.includes("/codeTypeChecking/")
  ) {
    return;
  }
        ...
        if (currLineTrimmed.startsWith("```")) {
          ...
          if (!startAppendingToCodeSnippet) {
            if (currLineTrimmed !== "```") {
              return rej(
                new Error(
                  `Something wrong in how a code snippet has ended in ${mdFile}`,
                ),
              );
            }
            // we just finished copying a code snippet
            if (currentCodeLanguage !== "ignore") {
              await addCodeSnippetToEnvHelper(
                currentCodeSnippet,
                currentCodeLanguage,
                mdFile,
                fileNameCounter,
            ...
            if (
              currLineTrimmed === "```js" ||
              currLineTrimmed.startsWith("```js ") ||
              currLineTrimmed === "```jsx" ||
              currLineTrimmed.startsWith("```jsx ")
            ) {
              return rej(
                new Error(
                  `Please do not use js or jsx in code snippets. Only ts or tsx. Error in ` +
                    mdFile,
                ),
              );
            } else if (
              currLineTrimmed === "```ts" ||
              currLineTrimmed.startsWith("```ts ") ||
              currLineTrimmed === "```tsx" ||
              currLineTrimmed.startsWith("```tsx ")
            ) {
              currentCodeLanguage = "typescript";
            } else if (
              currLineTrimmed === "```go" ||
              currLineTrimmed.startsWith("```go ")
            ) {
              currentCodeLanguage = "go";
            } else if (
              currLineTrimmed === "```python" ||
              currLineTrimmed.startsWith("```python ")
            ) {
              currentCodeLanguage = "python";
            } else if (
              currLineTrimmed === "```kotlin" ||
              currLineTrimmed.startsWith("```kotlin ")
            ) {
              ...
              currLineTrimmed.includes("bash") ||
              currLineTrimmed.includes("yaml") ||
              currLineTrimmed.includes("cql") ||
              currLineTrimmed.includes("sql") ||
              ...
              currLineTrimmed === "```dart" ||
              currLineTrimmed.startsWith("```dart ")
            ) {
              currentCodeLanguage = "dart";
            } else {
              return rej(
                new Error(
                  `UNABLE TO RECOGNISE LANGUAGE ${currLineTrimmed} in file ${mdFile}.`,
                ),
              );
            }
          }
...
async function deleteFilesWithoutErrorsTypescript(stdout) {
  ...
  files.forEach((file) => {
    ...
    if (
      !fileNames.includes(actualPath.replace(snippetsPathPrefix, "snippets/"))
    ) {
...
async function checkCodeSnippets(language) {
  ...
  if (language === "typescript") {
    await new Promise((res, rej) => {
      exec(
        "cd src/plugins/codeTypeChecking/jsEnv/ && npm run test",
        async function (err, stdout, stderr) {
          ...
          if (err) {
            ...
            console.log(
              "\x1b[36m%s\x1b[0m",
              `To setup a JS env, run the following (from v2 folder):
    ...
    await new Promise((res, rej) => {
      exec(
        "cd src/plugins/codeTypeChecking/goEnv/ && go mod tidy && go build ./...",
        function (err, stdout, stderr) {
          if (err) {
            ...
            console.log(
              "\x1b[36m%s\x1b[0m",
              `Make sure that you have go installed on your system and try this command again`,
            );
            console.log("==========================\n");
            return rej(err);
          }
    ...
    await new Promise((res, rej) => {
      exec(
        "cd src/plugins/codeTypeChecking/pythonEnv/ && ./checkSnippets.sh",
        async function (err, stdout, stderr) {
          ...
          if (err) {
            ...
            console.log(
              "\x1b[36m%s\x1b[0m",
              `To setup a python env, run the following (from v2 folder):
    ...
    await new Promise((res, rej) => {
      exec(
        "cd src/plugins/codeTypeChecking/kotlinEnv/ && ./gradlew build",
        async function (err, stdout, stderr) {
          if (err) {
            console.log("\x1b[31m%s\x1b[0m", stdout);
            console.log("\x1b[31m%s\x1b[0m", err);
            ...
            console.log(
              "\x1b[36m%s\x1b[0m",
              `Make sure that you have kotlin installed on your system and try this command again`,
            );
            console.log("==========================\n");
            return rej(err);
          }
    ...
    await new Promise((res, rej) => {
      exec(
        "cd src/plugins/codeTypeChecking/iosenv/ && set -o pipefail && xcodebuild -workspace iosenv.xcworkspace -scheme iosenv build CODE_SIGN_IDENTITY='' CODE_SIGNING_REQUIRED=NO -quiet | ./Pods/xcbeautify/xcbeautify",
        async function (err, stdout, stderr) {
          if (err) {
            console.log("\x1b[31m%s\x1b[0m", stdout);
            console.log("\x1b[31m%s\x1b[0m", err);
            ...
            console.log(
              "\x1b[36m%s\x1b[0m",
              `Make sure that you have Xcode installed on your system and try this command again`,
            );
            console.log(
              "\x1b[36m%s\x1b[0m",
              `Make sure that you have run 'pod install' inside iosenv and try this command again`,
            );
            console.log("==========================\n");
            return rej(err);
          }
    ...
    await new Promise((res, rej) => {
      exec(
        "cd src/plugins/codeTypeChecking/dart_env/ && flutter build web -t lib/snippets/main.dart",
        async function (err, stdout, stderr) {
          if (err) {
            console.log("\x1b[31m%s\x1b[0m", stdout);
            console.log("\x1b[31m%s\x1b[0m", err);
            ...
            console.log(
              "\x1b[36m%s\x1b[0m",
              `Make sure that you have flutter setup on your system and try this command again`,
            );
            console.log(
              "\x1b[36m%s\x1b[0m",
              `Make sure that you have run 'flutter pub get' inside dart_env and try this command again`,
            );
            console.log("==========================\n");
            return rej(err);
          }
...
async function addCodeSnippetToEnvHelper(
  codeSnippet,
  language,
  mdFile,
  ...
  codeSnippet = codeSnippet.replace(
    /__OMIT_START__([\s\S]*?)__OMIT_END__/g,
    "$1",
  );
  ...
  codeSnippet = codeSnippet.replaceAll(
    "^{coreInjector_api_key_commented_with_hash}",
    "",
  );
  ...
  codeSnippet = codeSnippet.replaceAll(
    "^{form_contactMethod_initialize_Python}",
...
Enabled: true,
  ...
  if (ogCodeSnippet !== codeSnippet) {
    for (let i = 0; i < 50; i++) {
      if (
        ...
        codeSnippet =
          "// THIS FILE CONTAINS DOCS VARIABLES. PLEASE DO NOT FORGET TO USE THOSE\n" +
          codeSnippet;
      } else if (language === "python") {
        codeSnippet =
          "# THIS FILE CONTAINS DOCS VARIABLES. PLEASE DO NOT FORGET TO USE THOSE\n" +
  ...
  if (language === "typescript") {
        ...
        async (err) => {
          if (err) {
            ...
            await assertThatUserIsNotRemovedDocsVariableByMistake(
              "src/plugins/codeTypeChecking/jsEnv/snippets/" +
                folderName +
                "/index.tsx",
              codeSnippet,
            );
    ...
    if (
      codeSnippet.includes("/supertokens-go/") ||
      codeSnippet.includes("/supertokens-go\n")
    ) {
      throw new Error(
        "Do not use supertokens-go package. Use supertokens-golang package. Error in " +
          mdFile,
      );
    }
    ...
    await new Promise(async (res, rej) => {
      fs.mkdir(
        "src/plugins/codeTypeChecking/pythonEnv/snippets/" + folderName,
        { recursive: true },
        async (err) => {
          if (err) {
            rej(err);
    ...
    await new Promise(async (res, rej) => {
      fs.mkdir(
        "src/plugins/codeTypeChecking/kotlinEnv/app/src/main/java/com/example/myapplication/" +
          folderName,
        { recursive: true },
        async (err) => {
          if (err) {
    ...
    const lastFolderPath =
      folderNameSplit[folderNameSplit.length - 1] + new Date().getTime();
    await new Promise(async (res, rej) => {
      fs.mkdir(
        "src/plugins/codeTypeChecking/iosenv/iosenv/Snippets/" + folderName,
        { recursive: true },
        async (err) => {
          if (err) {
            rej(err);
            ...
            await assertThatUserIsNotRemovedDocsVariableByMistake(
              "src/plugins/codeTypeChecking/iosenv/iosenv/Snippets/" +
                folderName +
                "/Code.swift",
              codeSnippet,
            );
            const filename =
              "src/plugins/codeTypeChecking/iosenv/iosenv/Snippets/" +
              folderName +
              `/${lastFolderPath}.swift`;
            fs.writeFile(filename, codeSnippet, function (err) {
              if (err) {
                rej(err);
              } else {
                execSync(
                  `./src/plugins/codeTypeChecking/iosenv/createFile.rb "${filename}"`,
                );
                res();
              }
            });
    ...
    await new Promise(async (res, rej) => {
      fs.mkdir(
        "src/plugins/codeTypeChecking/dart_env/lib/snippets",
        { recursive: true },
        async (err) => {
          if (err) {
            rej(err);
    ...
    await new Promise(async (res, rej) => {
      fs.writeFile(
        "src/plugins/codeTypeChecking/dart_env/lib/snippets/main.dart",
        mainContent,
        async (err) => {
          if (err) {
            rej(err);
    ...
    await new Promise(async (res, rej) => {
      fs.mkdir(
        "src/plugins/codeTypeChecking/dart_env/lib/snippets/" + folderName,
        { recursive: true },
        async (err) => {
          if (err) {
            rej(err);
...
async function assertThatUserIsNotRemovedDocsVariableByMistake(
  path,
  codeSnippet,
) {
  ...
  if (child.value) {
    ...
    for (let i = 0; i < eachLine.length; i++) {
      ...
      if (
        ...
        /(\/\/|#) typecheck-only, removed from output$/.test(line)
      ) {
        continue;
      }
      ...
      if (line.includes("supertokensUIInit(")) {
        line = line
          .split("supertokensUIInit(")
          .join('(window as any).supertokensUIInit("supertokensui", ');
        newLines.push(line);
        continue;
      }
      if (
        line.includes("supertokensUI") &&
        !line.includes("supertokens-auth-react-script")
      ) {
        line = line
          .split("supertokensUI")
          .join("(window as any).supertokensUI");
        newLines.push(line);
        continue;
      }
