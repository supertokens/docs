...
async function checkFrontendSDKRelatedDocs(mdPath) {
  ...
  return new Promise((res, rej) => {
    fs.readFile(mdPath, "utf8", async (err, data) => {
      ...
      let count =
        (data.match(new RegExp('<TabItem value="reactjs">', "g")) || [])
          .length +
      ...
      if (count > 0) {
        ...
        if (!mdPath.includes("pre-built-ui")) {
          ...
          if (data.indexOf("show_ui_switcher: true") === -1) {
            rej(
              new Error("show_ui_switcher: true is not in the file: " + mdPath),
            );
          }
        }
        ...
        let angularCount =
          (data.match(new RegExp('<TabItem value="angular">', "g")) || [])
            .length +
          (data.match(new RegExp("<TabItem value='angular'>", "g")) || [])
            .length;
        if (angularCount !== count) {
          rej(
            new Error(
              "The number of angular and reactjs tabs are not equal: " + mdPath,
            ),
          );
        }
      }
      ...
      let linesWithScript = lines.filter((line) =>
      ...
      if (linesWithScript.length > 0) {
        ...
        for (const line of linesWithScript) {
          if (!ALLOWED_LINES.includes(line.trim())) {
            rej(
              new Error(
                "The line is not allowed: " + line + " in the file: " + mdPath,
              ),
            );
          }
        }
    ...
    async loadContent() {
      ...
      await new Promise((res, rej) => {
        walk(__dirname + "/../../", async (err, results) => {
          ...
          results = results.filter(
            (r) => r.endsWith(".md") || r.endsWith(".mdx"),
          );
      ...
      if (check !== undefined && check !== "nothing") {
        ...
        if (isSwiftEnabled) {
          ...
          fs.readdirSync(
            ...
            fs.rmSync(
              "src/plugins/codeTypeChecking/iosenv/iosenv/Snippets/" + file,
              {
                recursive: true,
                force: true,
              },
      ...
      try {
        ...
        if (check !== undefined && check !== "nothing") {
          ...
          if (
            splittedCheck.filter((i) => i === "all").length >= 1 ||
            splittedCheck.filter((i) => i === "dart").length >= 1
          ) {
  ...
  return new Promise((res, rej) => {
    fs.readFile(mdFile, "utf8", async (err, data) => {
      ...
      if (containsBoth && ignore === undefined) {
        let originalDocsPath = path.resolve(
          __dirname +
            "/../../" +
            pathLine.replace(/ /g, "").replace("<!--", "").replace("-->", ""),
        );
      ...
      if (copyType === "DOCS") {
        ...
        let sourceData = await getDataToCopySectionFromFile(
          lines,
          copyPathsAndIDs,
        );
      ...
      fs.readFile(pathLine, "utf8", async function (err, toCopyData) {
        ...
        for (; finalDataIndex < finalData.length; finalDataIndex++) {
          if (finalData[finalDataIndex].trim() === "<!-- COPY SECTION -->") {
            if (
              finalData[finalDataIndex + 1].trim() === ogPathLine &&
              finalData[finalDataIndex + 2].trim() === ogId
            ) {
              finalDataIndex += 3;
              break;
            } else {
        ...
        finalData = [
          ...finalData.slice(0, finalDataIndex),
          ...finalData.slice(toRemoveIndex),
        ];
        ...
        for (let i = 0; i < copyLines.length; i++) {
          if (
            startCopying &&
            copyLines[i].trim() === "<!-- END COPY SECTION -->"
          ) {
            break;
          }
          if (
            copyLines[i].trim() === "<!-- COPY SECTION -->" &&
          ...
          finalData = [
            ...finalData.slice(0, finalDataIndex),
            copyLines[i],
            ...finalData.slice(finalDataIndex),
          ];
          finalDataIndex++;
        }
        res();
    ...
    (function next() {
      ...
      fs.stat(file, function (err, stat) {
        ...
        if (
          ...
          if (
            !name.endsWith("/v2/HOW_TO_NEW_DOCS.md") &&
            !name.endsWith("/v2/README.md")
          ) {
            results.push(file);
          }
          next();
