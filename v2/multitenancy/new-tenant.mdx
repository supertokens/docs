...
- We specify the appId for which we want to create a new tenant in the connectionURI of the core during `supertokens.init` (by providing the `/appid-<appId>` path in the connectionURI). If you are using the default (`"public"`) app, you can omit the `/appid-<appId>` part of the URL.
...
```bash
curl --location --request PUT '^{coreInjector_uri_without_quotes}/appid-<appId>/recipe/multitenancy/tenant' \
...
- We specify the appId for which we want to create a new tenant in the URL. If you are using the default (`"public"`) app, you can omit the `/appid-<appId>` part of the URL.
- In the above, we are creating a new tenant with the id `"customer1"`. We are also enabling the email password, third party and passwordless login for this tenant. You can also disable any of these by setting the corresponding field to `false` (which is also the default setting).
