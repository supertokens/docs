...
- If a user is required to complete a MFA challenge, for example TOTP, and they already have a registered TOTP device, we enforce that they cannot create another TOTP device unless they first complete all the required MFA challenges for logging in. This ensures that a user cannot bypass the MFA challenges by creating a new TOTP device (same goes for other email / sms OTP).
- When a user creates a new TOTP device, it cannot be used unless they first verify it by entering the inital TOTP code.
- If the email of the 2nd factor login method is not verified, by default, we do not allow it to be setup or used as a 2nd factor.
