...
We add `phoneNumberVerified: false` into the access token payload. This will indicate to the API routes and to the frontend that only the first factor has been completed. You can the key to some other name as well.