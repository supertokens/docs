...
- Finally, we check if the user has completed TOTP within the last 5 mins or not. If not, we show a message to the user, and ask them to complete TOTP.
    ...
    if (await Session.doesSessionExist()) {
        ...
        if (validationErrors.length === 0) {
            ...
            if (mfaClaimValue === undefined) {
                await MultiFactorAuthClaim.refresh({});
                return await shouldLoadRoute();
            }
...
- For checking for step up auth, we get the MFA claim value from the session (or refresh it if it doesn't exist), and then check if TOTP was completed within the last 5 mins. Only if it was, we return true, else we return false.
- Notice that we use a `DateProviderReference` class exported by SuperTokens instead of just doing `Date.now()`. This is done to take into account any clock skew that may exist between the frontend and the backend server.
...
async function shouldLoadRoute(): Promise<boolean> {
    if (await supertokensSession.doesSessionExist()) {
        ...
        if (validationErrors.length === 0) {
            ...
            if (mfaClaimValue === undefined) {
                await supertokensMultiFactorAuth.MultiFactorAuthClaim.refresh({});
                return await shouldLoadRoute();
            }
            let totpCompletedTime = mfaClaimValue.c["totp"];
