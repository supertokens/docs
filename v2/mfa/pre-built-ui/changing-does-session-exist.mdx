...
In order to achieve this, we will be overriding the `doesSessionExist` recipe function from the `Session.init` config. Internally, the `<SessionAuth>` wrapper and the `<Passwordless.SignInUp />` component both use the `doesSessionExist` function to determine if a redirection is required:
- `<SessionAuth>` wrapper: If `doesSessionExist` returns `false`, it will redirect the user to the auth screen, else it will keep the user on the current screen.
...
- `<SessionAuth>` wrapper: Redirects the user to the login screen if a session does not exist, or if they have not completed both the factors.
