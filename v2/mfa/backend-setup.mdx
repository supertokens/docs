...
Once you have enabled a factor for a user, you take them to that factor screen if they have not previously already setup the factor. To know if a factor is setup, you can call the following function: 
...
This is a special factor ID (`"access-denied"`) which can be present in the `n` array if the user still needs to complete certain factors, but can't. For example, TODO..