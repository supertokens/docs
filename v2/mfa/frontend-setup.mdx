...
- The function is called `resyncSessionAndFetchMFAInfo` because it does two things:
    ...
    - resyncs the value of the `v` and `c` in the session's MFA claim. As such, it's theoretically possible that the value of `v` changes when we call this function, so we should re-check that value once we call this API.
