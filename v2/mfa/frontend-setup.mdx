...
Once we determine that MFA is still pending, we need to get the list of factors the user must do next. We can do this by calling the MFA Info endpoint (see later) which returns a list of next (`string[]`) factors:
...
In the later guides of this recipe, we will walk through each of these use cases in detail. For now though, if you want to know the status of any factor, you can get that by calling the MFA Info endpoint (see later).
