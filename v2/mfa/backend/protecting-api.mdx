...
We can add this check to all APIs by overriding the `getGlobalClaimValidators` function in the Session recipe.
...
:::important
You can also use this method to remove a validator you have added to the global validators for a specific route.
:::
...
:::important
If the user has a session but hasn't finished both factors before calling an endpoint protected by the appropriate `SecondFactorClaim` validator, then the SDK will send a `403` response.
:::