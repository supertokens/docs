...
Next, we protect your APIs by overriding the `getGlobalClaimValidators` function in the Session recipe and adding a validator for `PhoneVerifiedClaim`:
...
You can disable this for a specific routes by providing the `overrideGlobalClaimValidators` option to `verifySession`

:::info
You can also use this method to protect specific APIs if you choose to not enable this checking globally.
:::
