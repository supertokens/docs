...
async def sign_in_post(api_options: APIOptions):
    # ... your code here ...
...
:::note
For using with other frameworks, you can follow the same steps as above to override the APIs and add custom logic for anomaly detection.
:::
...
async def handle_security_checks(request_id: Union[str, None], password: Union[str, None], brute_force_config: List[Dict[str, Any]], email: Union[str, None], phone_number: Union[str, None], action_type: str):
    request_body = {}
    ...
    response = requests.post(ANOMALY_DETECTION_API_URL, json=request_body)
