...
const supertokensDeviceIdPromise = import(SDK_URL + "?apiKey=" + API_KEY).then((RequestId: any) => RequestId.load({
    endpoint: [
        SDK_URL,
...
const supertokensDeviceIdPromise = import(SDK_URL + "?apiKey=" + API_KEY).then((DeviceId: any) => DeviceId.load({
    endpoint: [
        SDK_URL,
        DeviceId.defaultEndpoint
    ]
