...
To protect your backend routes, you will have to wrap your API function with the `withSession` utility. 
The function limits endpoint access only to authenticated requests. It will also add the `session` object in as an argument to your request handler. 
...
SuperTokens will automatically add a session claim validator globally. Each time you call `withSession` the SDK will check if the claim is present and if it has the correct value.
