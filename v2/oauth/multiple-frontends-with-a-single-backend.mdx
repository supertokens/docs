...
- `tokenEndpointAuthMethod` - Indicates that the client will not use a client secret in the process of obtaining the **OAuth2 Access Token**. 
...
Add the new import statements and update the list of recipes with the new initialization steps.
...
#### 3. Disable Network Interceptors
...
Since in the scenario that you are implementing **OAuth2 Access Tokens** are used for authorization. The automatic request interception will end up causing conflicts.
In order to prevent this you will have to override the `shouldDoInterceptionBasedOnUrl` function in the `Session.init` call.

:::info

The example below only allows interception for API endpoints that start with `/auth`. 
...
Use a generic OAuth2 library to initiate the login flow
