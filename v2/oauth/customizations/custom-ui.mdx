...
If you have skipped this step, please check custom UI our [guides](/docs) based on the authentication methods that you want to use.
...
## 2. Handle the Initial Login Flow 

Now that you know which route gets used for each action you can proceed to the actual frontend implementation.

In order to handle the initial login state you will have to redirect the user to a specific frontend route. This way the UI will end up rendering the correct authentication forms. 
...
// If you want to render additional information while the UI waits fo the redirection URL
// you can use the OAuth2Provider.getLoginChallengeInfo function
