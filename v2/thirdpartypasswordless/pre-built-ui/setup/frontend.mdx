...
- In your `auth` component folder create a react component `supertokensAuthComponent.tsx` where we initialize the `supertokens-auth-react` SDK. This will tell SuperTokens which UI to show when the user visits the login page.
    ...
    class SuperTokensReactComponent extends React.Component {
        override render() {
            if (ThirdPartyPasswordlessPreBuiltUI.canHandleRoute()) {
                return ThirdPartyPasswordlessPreBuiltUI.getRoutingComponent();
            }
            ...
            <SuperTokensWrapper>
                <BrowserRouter>
                    <Routes>
                        ...
                        // highlight-next-line
                        {ThirdPartyEmailPasswordPreBuiltUI.getReactRouterDomRoutes(reactRouterDom)}
                        {/*Your app routes*/}
                    </Routes>
                </BrowserRouter>
            </SuperTokensWrapper>
