...
- We could use this SDK for the entire app but it would inflate the bundle size with UI components and dependencies that most of our app wont use.
- Thats where the `supertokens-web-js` SDK comes in. It contains the functions to handle authentication and session management without any of the pre-built UI. In fact, the `supertokens-web-js` SDK is used internally by the `supertokens-auth-react` SDK.
