...
Start by installing the ReactJS SDK:
...
:::info NOTE
Notice that we've wrapped the entire application with `SuperTokensWrapper`. This is optional but provides session information in all of your components. Check [here](../common-customizations/get-user-info#on-the-frontend) to see how you can access it.
:::
...
**Initialize the `supertokens-auth-react` SDK in your `auth`  component.** This will tell SuperTokens which UI to show when the user visits the login page.
...
import { Github, Google, Facebook, Apple } from "supertokens-auth-react/recipe/thirdpartypasswordless";
import Session from "supertokens-auth-react/recipe/session";
...
You can skip this step as it's already done in the previous ones.
