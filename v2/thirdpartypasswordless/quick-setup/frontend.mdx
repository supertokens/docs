...
Before we initialize the `supertokens-auth-react` SDK lets see how we will use it in our Vue app

**Architecture**

- When a user visits the `^{form_websiteBasePath}` route we load a Vue component which initializes the `supertokens-auth-react` SDK.
- The `supertokens-auth-react` SDK is responsible for rendering login UI and handling authentication flows.
- Other routes in our Vue app will use the `supertokens-web-js` SDK to handle session management.
