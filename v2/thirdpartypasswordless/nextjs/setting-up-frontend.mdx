...
const SuperTokensComponentNoSSR = dynamic<React.ComponentProps<typeof ^{recipePreBuiltUINameCapitalLetters}.getRoutingComponent>>(
  new Promise((res) => res(^{recipePreBuiltUINameCapitalLetters}.getRoutingComponent)),
  { ssr: false }
)
...
export default function Auth() {
  ...
  useEffect(() => {
    if (^{recipePreBuiltUINameCapitalLetters}.canHandleRoute() === false) {
      redirectToAuth()
    }
  }, [])
