...
export function GET(request: NextRequest) {
  return withSession(request, async (session) => {
    // Session will be undefined if it does not exist
    if (!session) {
      // The frontend sdk will try to refresh the session if it recieves a 401 status code
      return new NextResponse('Authentication required', { status: 401 });
    }

    // The session object exposes utility functions to fetch information such as the user id
