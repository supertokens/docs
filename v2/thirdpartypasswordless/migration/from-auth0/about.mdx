...
- We will be overriding the SuperTokens login function so that if a user logs into your app with with their phone number or social account, we check if an associated passwordless Auth0 account exists, retrieve their userInfo and create a new SuperTokens account.
