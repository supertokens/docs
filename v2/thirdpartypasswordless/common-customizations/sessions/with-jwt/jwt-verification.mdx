...
There are two steps in doing session verification using JWTs:
...
When configured with `VIA_TOKEN`, an explicit `anti-csrf` token will be attached to requests. To verify the `anti-csrf` token you will need to compare it the to value of the `antiCsrfToken` key from the payload of the decoded JWT.

### Checking for anti-csrf when `VIA_CUSTOM_HEADER` is set
When configured with `VIA_CUSTOM_HEADER`, you will need to check for the presence of the `rid` header from incoming requests. 
