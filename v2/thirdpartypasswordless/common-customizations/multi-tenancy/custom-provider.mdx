...
Whilst there are [several built-in providers](../signup-form/provider-config) that SuperTokens offers, you may want to add your own provider. This page will show you how to do that on a per tenant basis so that you can add a custom enterprise connection or a social connection to each tenant.
