...
async function fetchLoginMethodsForTenant(tenantId: string) {
    ...
    if (loginMethods.passwordless.enabled) {
        // render sign in with magic link / otp UI as well.
    }
...
- We then render the login UI buttons based on the configured `thirdPartyId`s in the response and if the `passwordless` login method is enabled.
...
```tsx
import { getAuthorisationUrl } from "supertokens-web-js/recipe/thirdpartypasswordless";
...
```tsx
import supertokensThirdParty from "supertokens-web-js-script/recipe/thirdpartypasswordless";
async function handleUserClick(thirdPartyId: string, tenantId: string) {
    try {
...
- For passwordless login, it's the same steps as mentioned [on the regular passwordless login page](../../custom-ui/login-magic-link), except that when you are calling the sign in up API, you also need to pass in the tenantId.

</TabItem>

<TabItem value="mobile">

All the steps for mobile app login are similar to the [social login steps](../../custom-ui/thirdparty-login), and [passwordless login page](../../custom-ui/login-magic-link) except that when you are calling the APIs, you also need to pass in the tenantId in the request body. An example of this is shown below:
