...
async function fetchLoginMethodsForTenant(tenantId: string) {
	...
	if (loginMethods.firstFactors.contains("thirdparty")) {
	...
	if (loginMethods.firstFactors.contains("emailpassword")) {
...
async function fetchLoginMethodsForTenant(tenantId: string) {
	...
	if (loginMethods.firstFactors.contains("otp-email") || 
        loginMethods.firstFactors.contains("otp-sms") ||
        loginMethods.firstFactors.contains("link-email") ||
        loginMethods.firstFactors.contains("link-sms")) {
		// render sign in with passwordless UI as well.
	}
	// more checks for other login methods...
}
...
All the steps for mobile app login are similar to the [social login steps](../../custom-ui/thirdparty-login), and [passwordless login page](../../custom-ui/login-magic-link) except that when you are calling the APIs, you also need to pass in the tenantId in the request path. An example of this is shown below:
