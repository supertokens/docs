        ...
        apis: (originalImplementation) => {
            return {
                ...
                createCodePOST: async function (input) {
                    if ("email" in input) {
                        let existingUser = await ^{recipeNameCapitalLetters}.getUsersByEmail(input.email);
...
def override_^{codeImportRecipeName}_apis(original_implementation: APIInterface):
    ...
    async def create_code_post(email: Union[str, None], phone_number: Union[str, None],
                               api_options: PasswordlessAPIOptions, user_context: Dict[str, Any],
...
app.post("/create-user", verifySession({
    ...
    let inviteLink = await ThirdPartyPasswordless.createMagicLink({
        email
    });
...
func createUserAPI(w http.ResponseWriter, r *http.Request) {
	...
	inviteLink, err := thirdpartypasswordless.CreateMagicLinkByEmail(email)
	if err != nil {
