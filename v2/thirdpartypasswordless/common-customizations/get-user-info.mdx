...
```python
from supertokens_python.recipe.thirdparty.asyncio import get_users_by_email as get_users_by_email_third_party
from supertokens_python.recipe.passwordless.asyncio import get_user_by_email as get_user_by_email_passwordless

async def some_func():
    # Note that users_info has type List[User]
    third_party_user_info = await get_users_by_email_third_party("public", "test@example.com")

    passwordless_user_info = await get_user_by_email_passwordless("public", "test@example.com")
...
```python
from supertokens_python.recipe.passwordless.syncio import get_user_by_phone_number
