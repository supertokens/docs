...
func getUserInfoAPI(w http.ResponseWriter, r *http.Request) {
	...
	if userInfo == nil {
		// no third party user exists with the id, so now we check passwordless user instead
...
func getUserInfoAPI(w http.ResponseWriter, r *http.Request) {
	...
	if userInfo == nil {
		// no third party user exists with the id, so now we check passwordless user instead
		passwordlessdUserInfo, err := passwordless.GetUserByID(userID)
		if err != nil {
			// TODO: Handle error
			return
		}
		fmt.Println(passwordlessdUserInfo)
	} else {
		fmt.Println(userInfo)
	}
	// highlight-end
