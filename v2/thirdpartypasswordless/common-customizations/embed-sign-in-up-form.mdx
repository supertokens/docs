...
1. Disabled the default Auth UI by setting `disableDefaultUI` to `true` inside the ThirdPartyPasswordless recipe config.
2. Overrode the `getRedirectionURL` function inside the ThirdPartyPasswordless recipe config to redirect to `/dashboard` upon successful login.
