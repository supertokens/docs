...
- **6)** After the core replies with new session tokens, the backend SDK attaches them to the response as cookies or headers and sends them to the frontend. By default, the choice to use cookies or headers is controlled by the frontend.
...
- **1)** After signing in, the frontend has the access token and the refresh tokens as cookies. They are marked [`HttpOnly` cookies](https://owasp.org/www-community/HttpOnly) if you are using cookie-based sessions. The browser automatically stores and manages them. The access token is short lived, whilst the refresh token is long lived. 
...
- **6)** The API will call the core to generate a new access and a new refresh token. The API will then attach these to the response and sends them to the frontend. If the incoming request used cookies to send the tokens the response will use cookies as well, otherwise we use headers.
