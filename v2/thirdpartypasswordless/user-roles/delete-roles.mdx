---
sidebar_position: 1
title: Deleting roles
hide_title: true
---

<!-- COPY DOCS -->
<!-- ./userroles/delete-roles.mdx -->

import BackendSDKTabs from "/src/components/tabs/BackendSDKTabs";
import TabItem from '@theme/TabItem';
import PythonSyncAsyncSubTabs from "/src/components/tabs/PythonSyncAsyncSubTabs";
import CoreInjector from "/src/components/coreInjector"

# Deleting roles

You can delete any role you have created, if the role you are trying to delete does not exist then this has no  effect

:::tip
You can easily delete any role that you have created from the user management dashboard. To know more about how to manage your user roles and permissions from user management dashboard look <a href="/docs/userdashboard/managing-user-roles-and-permissions" target="_blank">here</a>
:::

<BackendSDKTabs enableCurl>
<TabItem value="nodejs">

```tsx
import UserRoles from "supertokens-node/recipe/userroles";

async function deleteRole() {
    // highlight-start
    // Delete the user role
    const response = await UserRoles.deleteRole("user");

    if (!response.didRoleExist) {
        // There was no such role
    }
    // highlight-end
}
```

</TabItem>
<TabItem value="go">

```go
import (
	"github.com/supertokens/supertokens-golang/recipe/userroles"
)

func deleteRole() {
	// highlight-start
	// Delete the user role
	response, err := userroles.DeleteRole("user", nil)
	if err != nil {
		// TODO: Handle error
		return
	}

	if response.OK.DidRoleExist == false {
		// There was no such role
	}
	// highlight-end
}
```

</TabItem>
<TabItem value="python">

<PythonSyncAsyncSubTabs>
<TabItem value="asyncio">

```python
from supertokens_python.recipe.userroles.asyncio import delete_role

async def delete_role_function():
	# highlight-start
    res = await delete_role("user")
    if res.did_role_exist:
        # The role actually existed
        pass
	# highlight-end

```

</TabItem>
<TabItem value="syncio">

```python
from supertokens_python.recipe.userroles.syncio import delete_role

def delete_role_function():
	# highlight-start
    res = delete_role("user")
    if res.did_role_exist:
        # The role actually existed
        pass
	# highlight-end

```
</TabItem>
</PythonSyncAsyncSubTabs>

</TabItem>

<TabItem value="curl">

<CoreInjector defaultValue="http://localhost:3567" showAppId={true} showTenantId={false}>

```bash
curl --location --request POST '^{coreInjector_uri_without_quotes}/recipe/role/remove' \
--header 'api-key: ^{coreInjector_api_key_without_quotes}' \
--header 'Content-Type: application/json; charset=utf-8' \
--data-raw '{
  "role": "admin"
}'
```

</CoreInjector>

</TabItem>
</BackendSDKTabs>
