...
const AdminRoute = (props: React.PropsWithChildren) => {
    return (
        <SessionAuth
...
function InvalidClaimHandler(props: React.PropsWithChildren) {
    ...
    if (sessionContext.invalidClaims.length > 0) {
        if (sessionContext.invalidClaims.some(i => i.validatorId === UserRoleClaim.id)) {
            ...
            // You need to handle other invalid claims that you may have
            // added, or you can show a generic error and ask the user to relogin.
            return <div>Something seems wrong. Please contact support.</div>
        }
    }
...
Finally, we check the result of the validation in the `InvalidClaimHandler` component which displays `"You cannot access this page because you are not an admin."` if the `UserRoleClaim` claim failed. In case some other validator failed (for example email verification valditor in case you have enabled email verification), then we display a `"Something seems wrong. Please contact support."` message. If this happens, it means that something is wrong with how you have done access control in your application.

Finally, if all validation passes, we render the `props.children` component.
...
function ProtectedComponent() {
    ...
    let roles = sessionContext.getClaimValue(UserRoleClaim)
    if (roles !== undefined && roles.includes("admin")) {
        // User is an admin
    } else {
...
async function shouldLoadRoute(): Promise<boolean> {
    if (await Session.doesSessionExist()) {
        // highlight-start
        let roles = await Session.getClaimValue(UserRoleClaim);
        if (roles !== undefined && roles.includes("admin")) {
            // User is an admin
            return true;
        }
...
async function shouldLoadRoute(): Promise<boolean> {
    if (await supertokensSession.doesSessionExist()) {
        // highlight-start
        let roles = await supertokensSession.getClaimValue(supertokensUserRoles.UserRoleClaim);
        if (roles !== undefined && roles.includes("admin")) {
            // User is an admin
            return true;
        }
