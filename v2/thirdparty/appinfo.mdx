...
For example, the default `apiBasePath` value is `/auth`, if it is changed to `/supertokens`, then your refresh endpoint will be updated from `/auth/session/refresh` to `/supertokens/session/refresh`. This results in previously issued refresh tokens not being sent to the new endpoint and the user will be logged out.
:::
