...
<!-- COPY DOCS -->
<!-- ./thirdpartyemailpassword/nextjs/app-directory/setting-up-backend.mdx -->
...
## 1) Create the `app/api/auth/[[...path]].tsx` page
- Be sure to create the `auth` folder in the `app/api/` folder.
- `[[...path]].tsx` will use the `getAppDirRequestHandler` helper function exposed by `supertokens-node` which helps in calling all the APIs like sign in, sign up etc..
- An example of this can be found [here](https://github.com/supertokens/next.js/blob/canary/examples/with-supertokens/app/api/auth/%5B%5B...path%5D%5D.ts).
