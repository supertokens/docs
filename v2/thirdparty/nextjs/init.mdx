...
## 6) Add session interceptor for `axios`
:::important
...
The interceptor calls the refresh endpoint automatically when the access token expires. For `fetch`, the interceptor is added automatically, when calling `SuperTokensReact.init`