...
## Free tier
The free tier of the managed service has a rate limit of 10 requests per second with a burst of 20 requests per second (with nodelay). This should be enough for 2 concurrent sign in / up (each sign in API call queries the SuperTokens core multiple times).
...
http {
    ...
    limit_req_zone $limit_req_zone_key zone=mylimit:10m rate=10r/s;
