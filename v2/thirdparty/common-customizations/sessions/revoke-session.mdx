...
:::caution
Note that enabling this feature may slow down all of your API calls which require session verification.
:::

For example, in the APIs defined in [Method 2](#method-2-revoking-a-session-using-a-sessionhandle), [Method 3](#method-3-revoking-multiple-sessions-using-an-array-of-sessionhandle) and [Method 4](#method-4-revoking-all-sessions-for-a-userid), if the access token blacklisting feature is enabled, these APIs will now result in the access token being cleared from the frontend.
