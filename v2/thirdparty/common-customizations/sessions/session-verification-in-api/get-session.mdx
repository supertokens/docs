...
```tsx
import { VerifySessionOptions } from "supertokens-node/recipe/session/types";
import { errorHandler, SessionContainer } from "supertokens-node/framework/express";
import Session from "supertokens-node/recipe/session";
import { Error as SuperTokensError } from "supertokens-node";

async function verifySession(options?: VerifySessionOptions) {
    let session: SessionContainer | undefined;
    try {
        if (options?.sessionRequired === false) {
            session = await Session.getSessionWithoutRequestResponse({
                antiCsrfCheck: options?.antiCsrfCheck,
                overrideGlobalClaimValidators: options?.overrideGlobalClaimValidators,
                sessionRequired: false
            });
...
func VerifySession(options *sessmodels.VerifySessionOptions) err {
    session, err := session.getSessionWithoutRequestResponse(r, w, options)
    if err != nil {
        if defaultErrors.As(err, &errors.TryRefreshTokenError{}) {
            // This means that the session exists, but the access token
            // has expired.
...
def verify_session(
    ...
    try:
        session = get_session_without_request_response(baseRequest,
                                anti_csrf_check,
                                session_required,
                                override_global_claim_validators)
    except Exception as e:
