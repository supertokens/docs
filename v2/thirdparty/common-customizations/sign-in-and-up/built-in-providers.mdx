...
:::important
Ensure that the Third Party provider returns `email` of the user for the scopes specified manually. Refer provider specific documentation to include the correct scope so that the provider returns the `email` of the user.
:::
