...
### Modifying `AuthorizationEndpointQueryParams` and `TokenEndpointBodyParams`
...
Sometimes for a custom provider, the built-in implementations may not be enough. For example, you may need to call multiple APIs to get the User info. In such cases, you can override the custom provider functions to provide your own implementations.
