...
function MyAuthPage() {
    ...
    return (
        <div>
            <Header />
            // highlight-next-line
            <AuthPage preBuiltUIList={[ThirdPartyPreBuiltUI]} navigate={navigate} />
            <Footer />
        </div>
    );
}
...
function MyAuthPage() {
    ...
    return (
        <div>
            <Header />
            // highlight-next-line
            <AuthPage preBuiltUIList={[ThirdPartyPreBuiltUI]} navigate={history} />
            <Footer />
        </div>
    );
}
...
1. Disabled the default Auth UI by setting `disableDefaultUI` to `true` inside the ThirdParty recipe config.
2. Override the `getRedirectionURL` function inside the ThirdParty recipe config to redirect to `/dashboard` upon successful login or to `/auth` when login is required.
