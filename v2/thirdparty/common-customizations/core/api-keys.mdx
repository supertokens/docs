...
API keys are used to indentify authentication/authorization requests from a client trying to query a SuperTokens core instance. If the API key is not specified in the client's backend config, auth related APIs to the core will return a `401` `UNAUTHORIZED` error. 
