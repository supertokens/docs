...
def override_thirdparty_apis(original_implementation):
	...
	async def sign_in_up_post(provider: Provider, code: str, redirect_uri: str, api_options: APIOptions):
		# First we call the original implementation of signInPOST.
		response = await original_sign_in_up_post(provider, code, redirect_uri, api_options)
