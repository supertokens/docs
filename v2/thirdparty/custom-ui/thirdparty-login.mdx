...
:::note
Refer [this page](./multi-tenancy) if you are implementing a multi-tenant SSO.
:::
...
:::important
This means, that the value provided to the `frontendRedirectURL` param in step 1, will be ignored and that on the Apple developer dashboard, you should set the `redirect_uri` to your API domain on the `/auth/callback/apple` route (where `/auth` is the value of `apiBasePath` set on the backend).
:::
...
TODO update this to talk about clientType instead of clientId.
