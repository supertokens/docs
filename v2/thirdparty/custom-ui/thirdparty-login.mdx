...
```tsx
import { ^{webjsGetAuthorisationUrl} } from "supertokens-web-js/recipe/thirdparty";
...
async function googleSignInClicked() {
    try {
        const authUrl = await supertokensThirdParty.^{webjsGetAuthorisationUrl}({
            thirdPartyId: "google",
...
from supertokens_python.recipe.thirdparty.interfaces import APIInterface, ^{pythonThirdPartyAPIOptions}
