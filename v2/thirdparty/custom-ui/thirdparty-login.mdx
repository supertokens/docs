...
In order to tell SuperTokens which frontend route to redirect the user back to, you need to set the `frontendRedirectURL` to the frontend route (just like for other providers), and also need to set the `redirectURIOnProviderDashboard` to your backend API route (to which Apple will send a POST request).
