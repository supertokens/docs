...
CREATE TABLE IF NOT EXISTS oauth2_authcode(
    ...
    CONSTRAINT oauth2_authcode_pkey PRIMARY KEY(authorization_code_hash),

    CONSTRAINT oauth2_authcode_client_id_fkey FOREIGN KEY(client_id) references oauth2_client(client_id) ON DELETE CASCADE,
);

CREATE INDEX oauth2_authcode_client_id_index ON oauth2_authcode(client_id);
CREATE INDEX oauth2_authcode_session_handle_index ON oauth2_authcode(session_handle);
CREATE INDEX oauth2_authcode_expired_at_ms_index ON oauth2_authcode(expired_at_ms);
...
CREATE TABLE IF NOT EXISTS oauth2_token(
    ...
    CONSTRAINT oauth2_token_client_id_fkey FOREIGN KEY(client_id) references oauth2_client(client_id) ON DELETE CASCADE,
);

CREATE INDEX oauth2_token_client_id_index ON oauth2_token(client_id);
CREATE INDEX oauth2_token_user_id_index ON oauth2_token(user_id);
CREATE INDEX oauth2_token_session_handle_index ON oauth2_token(session_handle);
...
CREATE TABLE IF NOT EXISTS oauth2_client(
    ...
    CONSTRAINT oauth2_client_pkey PRIMARY KEY(client_id)
);
...
CREATE TABLE IF NOT EXISTS oauth2_client_allowed_scopes(
    ...
    CONSTRAINT oauth2_client_allowed_scopes_pkey PRIMARY KEY(client_id, scope)
)

CREATE TABLE IF NOT EXISTS oauth2_scopes(
    scope TEXT NOT NULL,
    
    CONSTRAINT oauth2_scopes_pkey PRIMARY KEY(scope)
);
```
