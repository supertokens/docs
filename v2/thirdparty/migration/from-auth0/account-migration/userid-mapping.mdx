...
For example, if you have followed the steps mentioned in the [Modifications to Login](./modifications-to-login) section, when a user signs in with their Auth0 account credentials, they will be able to login without having to create a new account. Additionally their `userIds` will be mapped. This means that recipe functions can now take the Auth0 `userId` and will also return the Auth0 `userId` in the response. Functions like `getUserById` can now take the Auth0 `userId` as input and functions like `getUsersByEmail` and `getUserByThirdPartyInfo` will return the Auth0 `userId` in their response if the mapping exists.
