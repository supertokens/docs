...
init(
    ...
    recipe_list=[
        session.init(
            override=override=session.InputOverrideConfig(functions=session_functions_override),
        )
    ]
)
```
...
async def set_role_endpoint(session: SessionContainer = Depends(verify_session())):
