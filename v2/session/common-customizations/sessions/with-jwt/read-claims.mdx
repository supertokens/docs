...
async function getName() {
      if (await Session.doesSessionExist()) {
            ...
            let name = (await Session.getAccessTokenPayloadSecurely()).name;
      }
}
```
...
async function getName() {
      if (await supertokensSession.doesSessionExist()) {
            ...
            let name = (await supertokensSession.getAccessTokenPayloadSecurely()).name;
      }
}
```
...
async function getName() {
      if (await SuperTokens.doesSessionExist()) {
            ...
            let name = (await SuperTokens.getAccessTokenPayloadSecurely()).name;
      }
}
```
</TabItem>
