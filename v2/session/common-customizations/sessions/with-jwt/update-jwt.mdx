...
- Using that object, we call the `mergeIntoAccessTokenPayload` with new content. This content completely overrides the existing object, that's why we first get the `currAccessTokenPayload` info.
