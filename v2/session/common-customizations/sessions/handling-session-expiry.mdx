...
The SDK will fire an `UNAUTHORISED` event when there is no valid session. You can listen to this event by passing the [`onHandleEvent` function](../../advanced-customizations/frontend-hooks/handle-event) in the config like so:
