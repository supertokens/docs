...
app.post("/login", async (req, res) => {
    ...
    let session = await Session.createNewSessionWithoutRequestResponse(userId);
...
func login(w http.ResponseWriter, r *http.Request) {
	...
	session, err := session.createNewSessionWithoutRequestResponse(userId, nil, nil)
	if err != nil {
		err = supertokens.ErrorHandler(err, r, w)
		if err != nil {
			// Send 500 to client
...
async def login(request: Request):
    ...
    session = await create_new_session_without_reqyest_response(user_id)
