...
### 1️) Complete the [Quick setup guide](/docs/session/quick-setup/frontend)
Make sure you have completed the frontend, backend and SuperTokens core setup.

### 2️) Use the `override` config in `Session.init()`
...
### 4️) Retrieve the Fauna user token in any API
...
```jsx
let Session = require("supertokens-node/recipe/session/framework/express");
...
### 5️) Retrieve the Fauna user token on the frontend
