...
Update the backend config, created in [step 2](./backend-config#3-create-a-backend-config-function), to enable the JWT feature.
...
The config above enables JWTs being created for sessions and adds the `aud` claim to each JWT.
...
- Enter a name for your authorizer
...
- Set a value for the "Audience" field, this will be the value you expect the JWT to have under the `aud` claim. In the backend config above the value is set to `"users"`
