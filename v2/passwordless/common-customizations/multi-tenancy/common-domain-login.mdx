...
If you have [followed the pre built UI setup](../../pre-built-ui/setup/frontend), when you visit the login screen, you should see the passwordless login UI. We want to change the flow so that the user is first asked to enter their tenant ID and then the login UI is shown based on the tenant ID.
...
Now if you visit the login page, you will no longer see the passwordless component. We will now create our own login route on which we will display the following component:
...
```tsx
import React from 'react';
import { SignInAndUp } from "supertokens-auth-react/recipe/passwordless";
...
<!-- COPY SECTION -->
<!-- ./passwordless/common-customizations/multi-tenancy/common-domain-login.mdx  -->
<!-- 1 -->
...
When the user clicks on the passwordless sign in / sign up button, we call the relevant API along with the user's `tenantId`. For example, here is how we add the `tenantId` to the magic link generation function call:
...
async function sendMagicLink(email: string, tenantId: string) {
    try {
...
All the steps for mobile app login are similar to the [passwordless login steps](../../custom-ui/login-magic-link), except that when you are calling the APIs, you also need to pass in the tenantId in the request body. An example of this is shown below:
