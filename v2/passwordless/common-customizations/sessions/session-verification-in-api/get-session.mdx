...
async function verifySession(accessToken: string, antiCsrfToken?: string, options?: VerifySessionOptions) {
    ...
    try {
        if (options?.sessionRequired === false) {
            await Session.getSessionWithoutRequestResponse(accessToken, antiCsrfToken, {
                ...options,
                sessionRequired: false
            });
        } else {
            session = await Session.getSessionWithoutRequestResponse(accessToken, antiCsrfToken, {
                ...options,
                sessionRequired: true
            });
        }
