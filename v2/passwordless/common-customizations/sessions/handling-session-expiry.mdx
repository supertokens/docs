...
When you use `PasswordlessAuth` to provide `SessionContext` to your components,
...
const App = () => {
    return (
        <PasswordlessAuth onSessionExpired={showSessionExpiredPopup}>
            <MyComponent />
        </PasswordlessAuth>
    );
}
...
Read more about `PasswordlessAuth` and `onSessionExpired` in [its API reference](/docs/auth-react/passwordless/passwordless-auth).
