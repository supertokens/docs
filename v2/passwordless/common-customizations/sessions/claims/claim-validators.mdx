...
- Using the `onFailureRedirection` callback, you can choose to redirect the user to a specific url or path if the claim validation fails.
- By setting `showAccessDeniedOnFailure` to false, you can choose to still show the contents of SessionAuth even if the claim validation fails.
...
In case validation fails, you can choose to render an access denied screen, custom UI or redirect the user. In certain claim validators, like the email verification validator, the frontend SDK (for pre-built UI) automatically redirects the user to the email verification screen, but the default is to show an access denied screen if a claim validation fails.
...
1. Show an access-denied screen. This can be disabled by adding the `useDefaultAccessDeniedScreen={false}` prop to `SessionAuth` or you can override this screen by adding an override for `SessionAccessDenied_Override`.
...
const AdminRoute = (props: React.PropsWithChildren<any>) => {
    return (
        <SessionComponentsOverrideProvider 
            ...
            <SessionAuth 
                useDefaultAccessDeniedScreen={
                    /* 
                        If this is set to false, we render the `props.children` component even
                        if a claim validation failed (if we do not redirect)
                    */
                    true
                }
                overrideGlobalClaimValidators={(globalValidators) => [
...
By default, the SDK will render an access denied screen instead of the children passed to `SessionAuth` if a claim validation has failed. You can override this behaviour in a few ways.

#### Disabling for a specific route

You can disable the access denied screen on specific routes by setting `useDefaultAccessDeniedScreen` to false in the `SessionAuth` props. This will render `props.children` even if a session validation has failed. In this case, you can handle the invalid claims by checking the `invalidClaims` prop in the session context.
...
#### Disabling for a specific validator
...
If a claim validation failure required neither redirection nor showing the access denied screen (if `showAccessDeniedOnFailure` or `useDefaultAccessDeniedScreen` was set to false and `onFailureRedirection` is undefined or returned undefined), the `SessionAuth` component will render the child components. In this case, you can handle failing claim validators in the children by accessing the `invalidClaims` prop of the session context (accessed by `useSessionContext`):
