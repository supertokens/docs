...
In case validation fails, you can choose to render an access denied screen, custom UI or redirect the user. In certain claim validators, like the email verification validator, the frontend SDK (for pre-built UI) automatically redirects the user to the email verification screen.
...
1. Show an access-denied screen. This can be disabled by adding `useDefaultAccessDeniedScreen={false}` or you can override this screen by adding an override for `SessionAccessDenied_Override`.
