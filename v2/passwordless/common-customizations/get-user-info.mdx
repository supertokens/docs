...
async function handler() {
    const userInfo = await ^{recipeNameCapitalLetters}.^{getUserByEmailNode}({email: "test@example.com", tenantId: "public"});
}
...
async function handler() {
    const userInfo = await ^{recipeNameCapitalLetters}.getUserByPhoneNumber({phoneNumber: "+1234567891", tenantId: "public"});
}
```
