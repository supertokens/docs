...
1. Disabled the default Auth UI by setting `disableDefaultUI` to `true` inside the Passwordless recipe config.
2. Overrode the `getRedirectionURL` function inside the Passwordless recipe config to redirect to `/dashboard` upon successful login.
3. Overrode the `getRedirectionURL` function inside the SuperTokens config to redirect to `/auth` when login is required.
