...
You may choose to display the Auth UI in a full page and in a popup in the same app, utilising the same SuperTokens config. For the full-page Auth UI, redirection after a successful login is preferred, whereas in the popup, it is not. In this scenario, you have the option to check either the current URL or the `userContext` object in `getRedirectionURL` to decide whether to return null or not.