...
#### Disable email verification for specific routes
...
### Add checking to a single route
...
- We call the `validateClaims` function with the `EmailVerificationClaim` validator which makes sure that the user has a verified email address.
...
- This can include validators added by other recipe, but you can override that using the `overrideGlobalClaimValidators` option.
