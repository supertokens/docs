...
func createUserAPI(w http.ResponseWriter, r *http.Request) {
    ...
    // This will create the user in supertokens if they don't already exist.
    tenantId := "public"
    passwordless.^{goPasswordlessSignInUp}(tenantId, email)
...
from supertokens_python.recipe.passwordless.asyncio import create_magic_link, ^{pythonPasswordlessSignInUp}
...
from supertokens_python.recipe.userroles import UserRoleClaim
from supertokens_python.recipe.passwordless.syncio import create_magic_link, ^{pythonPasswordlessSignInUp}
