...
In your angular app's root component (where we initialize the `supertokens-auth-react` SDK) add the `EmailVerification` recipe to the `recipeList`.
...
In your vue app's root component (where we initialize the `supertokens-auth-react` SDK) add the `EmailVerification` recipe to the `recipeList`.
