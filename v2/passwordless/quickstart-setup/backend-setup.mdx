...
import PasswordlessQuickstartBackendCustomMagicLink from '../../community/reusableMD/passwordless-quickstart-backend-custom-magic-link.mdx'

import { PasswordlessBackendForm } from "/src/components/snippetConfigForm/passwordlessBackendForm";
import { ConditionalSection } from "/src/components/snippetConfigForm";
...
For the **Passwordless** recipe, you will also need to specify the `flowType` and `contactMethod`.
...
<ConditionalSection
  conditions={{ flowType:"MAGIC_LINK" }}
>
  
<PasswordlessQuickstartBackendCustomMagicLink />

</ConditionalSection>

<ConditionalSection
  conditions={{ flowType: "USER_INPUT_CODE_AND_MAGIC_LINK"}}
>
