...
In this section we will be modifying login to enable the migrating of users with Passwordless accounts from an external auth provider to SuperTokens.
...
:::tip
If you have not stored your user's  pre-existing `userId` in your database you can **ignore** the steps mentioned in **Box 7** and **Box 8** of the flow diagram.
:::

<img alt="Flow of login to migrate users with passwordless accounts from an external provider to SuperTokens" src="/img/thirdpartyemailpassword/ep-account-migration-agnostic.png" />
...
async function getUserInfoFromProvider(input :{
    email: string | undefined,
     phoneNumber: string | undefined }){
    // Box 2: Get userInfo from Provider with phone number or email.
        ...
        functions: (originalImplementation) => {
            return {
                ...
                consumeCode: async (input) => {
                    ...
                    // Box 2: retrieve the user data with the input email and phone number from the external auth provider
                    let auth0UserInfo = await getUserInfoFromProvider({
                        email: codeResponse?.email,
                        phoneNumber: codeResponse?.phoneNumber
                    })

                    // Box 3: check if a user with the email or phoneNumber exists in the external auth provider
                    if (auth0UserInfo !== undefined) {
                        ...
                        if (response.createdNewUser) {
                            // Box 6: map the user's provider userId to their SuperTokens userId, to learn more about userId mapping please check the User Id  Mapping section.
                            // If you have not stored the users provider userId in your tables, you can ignore this step
                            ...
                            // Box 8: Set the userId in the response to use the provider userId, to learn more about user mapping please check the User Id Mapping section.
                            // If you have not stored the provider userId, you can ignore this step.
                            response.user.id = auth0UserInfo.user_id
                        }
                        
                        // Box 9: return response
                        return response

                    } else {
                        // Box 10: User does not exist in the external provider, call the default implementation 
...
## Validating Providers credentials

In order to make the changes mentioned above, you have to now decide how you would like to validate the user's account credentials from the original provider.

There are two methods of validating user credentials:
- By calling your providers APIs
- Using the exported user data and password hashes from your provider.

<Tabs
    groupId="ep-migration-methods"  
    defaultValue="migration-method-1"
    values={[
        {label: "Using your Providers APIs", value: 'migration-method-1'},
        {label: 'Using the exported user data and password hashes. ', value: 'migration-method-2'},
]}>

<TabItem value="migration-method-1">

### Using your Providers APIs
