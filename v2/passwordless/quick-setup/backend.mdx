...
An example implementation can be [found here](https://github.com/supertokens/supertokens-auth-react/blob/master/examples/with-passwordless/api-server.js).
...
supertokens.init({
    ...
    recipeList: [
        Passwordless.init(), // initializes the passwordless signin features
...
supertokens.init({
    ...
    recipeList: [
        Passwordless.init(), // initializes signin / sign up features 
        Session.init() // initializes session features
    ]
});
```
...
## 7) Customize the link/code delivery method

:::caution Only for development
The built-in email service is not intended for production use. 
:::

By default, we use magic links (tokens) delivered by emails to log the users in. While the built-in email service is not intended for production use, you can use your favorite one: look [here](../common_customizations/customize-email) to see how you can customize both your email delivery service and the email templates. You can also go for text messages instead of emails see [this guide](../advanced_customizations/passwordless-by-text-message) for example integration with Twilio.