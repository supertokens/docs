...
- We add the `UserRoleClaim` validator to the `<SessionAuth>` function which makes sure that the user has an `admin` role.
- The `globalValidators` represents other validators that apply to all routes protected by `SessionAuth`. This may include a validator that enforces that the user's email is verified (if enabled by you).
- We can also add a `PermissionClaim` validator to enforce a permission.
