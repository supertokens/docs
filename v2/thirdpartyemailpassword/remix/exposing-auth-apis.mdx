...
To do so, within the `app` folder, create a `lib` folder (if one does not exist), and create an `authAPIRequestHandler.ts` file.

Paste the following code into `authAPIRequestHandler.ts`:

```tsx title="/app/lib/authAPIRequestHandler.ts"
...
Your `app/lib` folder should resemble the following:
...
For `authAPIRequestHandler.ts` to work, you must create a type definition file called `superTokensTypes.ts` located in the `app/lib` folder.
...
We'll add more types to this file later.
