			...
			^{codeImportRecipeName}.Init(&^{goModelName}.TypeInput{
				...
				Override: &^{goModelName}.OverrideStruct{
					APIs: func(originalImplementation ^{goModelName}.APIInterface) ^{goModelName}.APIInterface {
						...
						//highlight-start
						// we override the thirdparty sign in / up API
						(*originalImplementation.ThirdPartySignInUpPOST) = func(provider tpmodels.TypeProvider, code string, authCodeResponse interface{}, redirectURI string, options tpmodels.APIOptions, userContext supertokens.UserContext) (^{goModelName}.ThirdPartyOutput, error) {

							// first we call the original implementation
							resp, err := originalThirdPartySignInUpPOST(provider, code, authCodeResponse, redirectURI, options, userContext)
							if err != nil {
								return ^{goModelName}.ThirdPartyOutput{}, err
							}
...
from supertokens_python.recipe.^{codeImportRecipeName}.interfaces import APIInterface, ThirdPartyAPIOptions
