...
:::caution
Note that setting a custom `apiBasePath` will update the refresh API path, this can cause an issue where previously issued refresh tokens will no longer get sent to the new API endpoint and the user will be logged out. 

For example, the default `apiBasePath` value is "/auth", if it is changed to "/supertokens", then your refresh endpoint will be updated from "/auth/session/refresh" to "/supertokens/session/refresh". This results in your previously issued refresh tokens not being sent to the new API endpoint because they are associated with "/auth/session/refresh", causing a user logout.
:::
