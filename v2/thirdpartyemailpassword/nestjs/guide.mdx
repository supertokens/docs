...
Create a `config.interface.ts` file in the `auth` folder. We will put the type and injection token for the SuperTokens configuration here.

```ts title="./src/config.interface.ts"
...
Scaffold the middleware by running `nest g middleware auth auth` in the application's root folder.
...
```ts title="./src/auth/auth/auth.middleware.ts"
...
You can scaffold the exception filter using the CLI by running: `nest g filter auth auth`. This will result in a new `auth.filter.ts` file next to `auth.module.ts`. Edit this file to add the error handler:

```ts title="./src/auth/auth/auth.filter.ts"
...
Now that the library is set up, you can add a guard to protect your API. You can scaffold this by running: `nest g guard auth auth`. 

In the newly created `auth.guard.ts` file, implement session verification:

```ts title="./src/auth/auth/auth.guard.ts"
