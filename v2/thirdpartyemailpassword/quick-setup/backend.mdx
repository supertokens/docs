...
:::important
- Please make sure to replace all the <BackendSDKCasing>appInfo</BackendSDKCasing> configurations values with yours.
- To learn more about filling in <BackendSDKCasing>appInfo</BackendSDKCasing>, please visit [the appInfo page](../appinfo)
- If you would like to configure a specific path for you APIs, you can do so by setting the <BackendSDKCasing>apiBasePath</BackendSDKCasing> value in the config. For example, if your API path is `www.example.com/api` then you can set the value of <BackendSDKCasing>apiBasePath</BackendSDKCasing> to `/api`.
:::
...
In general, the OAuth callback URL is `{websiteDomain}{websiteBasePath}/callback/{id}` where `id` is the unique id for the auth provider, and `websiteBasePath` by default is `/auth`.
...
## 3) Add the SuperTokens middleware & CORS setup
