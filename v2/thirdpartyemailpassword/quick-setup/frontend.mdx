...
- Other routes in our Angular app will use the `supertokens-website` to handle session management.
...
```tsx {5} title="/auth/supertokensAuthComponent.tsx"
...
```tsx {29-31} title="auth/auth.component.ts"
...
```tsx {5-8} app/app-routing.modult.ts
...
```tsx {7-31} title="/auth/supertokensAuthComponent.tsx"
import * as React from "react";
import * as SuperTokens from "supertokens-auth-react";
...
```tsx {5-10} title="/auth/supertokensAuthComponent.tsx"
import React from 'react';
import SuperTokens from "supertokens-auth-react";
