...
- Our Angular app will have an `auth` route. To authenticate a user we will redirect them to auth route.
- This Angular component will load a react component and initialize the `supertokens-auth-react` SDK.
- Other routes in our Angular app will use the `supertokens-website` to handle session management.
- This is done to optimize loading times for all our other routes in our angular app by decreasing bundle sizes
...
Create an `auth` route which will load a React component when you visit the route.
...
- Create a react component where we initialize the `supertokens-auth-react` SDK.

```tsx {5} title="supertokensAuthComponent.tsx"
...
```tsx {7-31} title="supertokensAuthComponent.tsx"
import * as React from "react";
import * as SuperTokens from "supertokens-auth-react";
...
```tsx {5-10} title="supertokensAuthComponent.tsx"
import React from 'react';
import SuperTokens from "supertokens-auth-react";
