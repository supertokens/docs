...
:::info
This is only needed because currently `SessionAuth` does not support server side rendering, this will be fixed in a future version of the SDK and this step can be skipped after that.
:::
...
export async function HomePage() {
     ...
     * 
     * hasInvalidClaims indicates that some of the session claims have failed validation, for example if email
     * verification is required but the user has not verified their email.
     */
    if (!session) {
        if (!hasToken) {
            /**
