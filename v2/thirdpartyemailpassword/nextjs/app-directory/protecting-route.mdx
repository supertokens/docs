...
async function getSession(): Promise<{ session: SessionContainer | undefined; hasToken: boolean; hasInvalidClaims: boolean }> {
    ...
    return { session, hasToken, hasInvalidClaims };
}

export async function HomePage() {
    const { session, hasToken, hasInvalidClaims } = await getSession();
