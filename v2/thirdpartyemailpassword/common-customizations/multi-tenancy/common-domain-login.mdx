...
- We then render the login UI buttons based on the configured `thirdPartyId`s in the response and if the `emailPassword` login method is enabled.
...
```tsx
import { getAuthorisationUrl } from "supertokens-web-js/recipe/thirdpartyemailpassword";
...
```tsx
import supertokensThirdParty from "supertokens-web-js-script/recipe/thirdpartyemailpassword";
async function handleUserClick(thirdPartyId: string, tenantId: string) {
    try {
...
- For email password login, it's the same steps as mentioned [on the regular email password login page](../../custom-ui/email-password-login), except that when you are calling the sign in up API, you also need to pass in the tenantId.

</TabItem>

<TabItem value="mobile">

All the steps for mobile app login are similar to the [social login steps](../../custom-ui/thirdparty-login), and [email password login page](../../custom-ui/email-password-login) except that when you are calling the sign in up API, you also need to pass in the tenantId in the request body. An example of this is shown below:
