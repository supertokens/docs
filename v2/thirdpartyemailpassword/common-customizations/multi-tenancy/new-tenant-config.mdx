...
func main() {
  ...
  // highlight-start
	tenant, err := multitenancy.GetTenant("customer1")
  // highlight-end
    ...
    isEmailPasswordLoginEnabled := resp.EmailPassword.Enabled;
    isThirdPartyLoginEnabled := resp.ThirdParty.Enabled;
    isPasswordlessLoginEnabled := resp.Passwordless.Enabled;

    configuredThirdPartyProviders := resp.ThirdParty.Providers;
	}
}
```
