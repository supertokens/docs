...
The frontend setup is similar to that of a regular [single tenant social login page](../../custom-ui/thirdparty-login) and [email password login](../../custom-ui/email-password-login), but on page load, you will first need to check which third party providers are enabled for the current tenant. This can be done by querying the backend as shown below:
...
When the user clicks on a specific provider, you would need to follow the same steps mentioned in the [single tenant social login page](../../custom-ui/thirdparty-login) and [single tenant email password login](../../custom-ui/email-password-login). The only addition is that when calling `getAuthorisationURLWithQueryParamsAndSetState`, or the email password sign in / up functions, you would also need to pass in the `tenantId`.
