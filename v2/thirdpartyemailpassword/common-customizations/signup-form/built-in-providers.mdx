...
def override_apis(original_implementation):
	...
	async def thirdparty_sign_in_up_post(provider: Provider, code: str, redirect_uri: str, api_options: APIOptions):
		# First we call the original implementation of sign_in_up_post.
		response = await original_thirdparty_sign_in_up_post(provider, code, redirect_uri, api_options)
