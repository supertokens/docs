...
from supertokens_python.recipe.thirdpartyemailpassword.interfaces import APIInterface, EmailPasswordAPIOptions, EmailPasswordSignUpPostOkResponse
...
def override_apis(original_implementation: APIInterface):
    ...
    async def emailpassword_sign_up_post(form_fields: List[FormField],
        ...
        # Post sign up response, we check if it was successful
        if isinstance(response, EmailPasswordSignUpPostOkResponse):
