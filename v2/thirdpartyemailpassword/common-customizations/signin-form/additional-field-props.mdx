...
<!-- COPY DOCS -->
<!-- ./emailpassword/common-customizations/signin-form/additional-field-props.mdx -->
...
When you try to submit login form without a required/non-optional field, the app will throw an error `Field is not optional`.
...
Observe how the password field showed the standard error message because a custom message wasn't assigned using `nonOptionalErrorMsg` for this field.

:::tip

To display an error message for required/non-optional fields, make use of the `nonOptionalErrorMsg` property.
For complex validations of fields, make use of [field validators](/docs/emailpassword/common-customizations/signin-form/field-validators). 
