...
func changePasswordAPI(w http.ResponseWriter, r *http.Request) {
	...
	// get the signed in user's email from the getUserById function
	userInfo, err := ^{rid}.^{goGetUserById}(userID, nil)
	if err != nil {
		// TODO: Handle error
		return
	}
	...
	isPasswordValid, err := ^{rid}.SignIn(userInfo.Email, requestBody.OldPassword, nil)
	if err != nil {
		// TODO: Handle error
		return
	}
	...
	_, err = ^{rid}.UpdateEmailOrPassword(userID, &userInfo.Email, &requestBody.NewPassword, nil)
	if err != nil {
		// TODO: Handle error
		return
	}
...
def change_password():
    ...
    isPasswordValid = sign_in(users_info.email, request.json["oldPassword"]) # type: ignore
