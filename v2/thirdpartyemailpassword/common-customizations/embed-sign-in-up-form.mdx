...
1. Disabled the default Auth UI by setting `disableDefaultUI` to `true` inside the ThirdPartyEmailPassword recipe config.
2. Override the `getRedirectionURL` function inside the ThirdPartyEmailPassword recipe config to redirect to `/dashboard` upon successful login and to redirect to `/auth` when login is required.
