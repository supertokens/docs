---
id: embed-sign-in-up-form
title: Embed Sign In / Up form in a page
hide_title: true
show_ui_switcher: true
---
import {PreBuiltOrCustomUISwitcher, PreBuiltUIContent, CustomUIContent} from "/src/components/preBuiltOrCustomUISwitcher"
import FrontendPreBuiltUITabs from "/src/components/tabs/FrontendPreBuiltUITabs"
import TabItem from '@theme/TabItem';
import {Question, Answer}from "/src/components/question"
import RRDVersionSubTabs from "/src/components/tabs/RRDVersionSubTabs"

# Embed Sign In / Up form in a page

<PreBuiltOrCustomUISwitcher>

<PreBuiltUIContent>

## Step 1: Disable default implementation

This will prevent SuperTokens from displaying the default login UI in the `/auth` page.

<FrontendPreBuiltUITabs>
<TabItem value="reactjs">

```tsx
import SuperTokens from "supertokens-auth-react";
import ThirdPartyEmailPassword from "supertokens-auth-react/recipe/thirdpartyemailpassword";

SuperTokens.init({
    appInfo: {
        apiDomain: "...",
        appName: "...",
        websiteDomain: "..."
    },
    recipeList: [
        ThirdPartyEmailPassword.init({
            signInAndUpFeature: {
                // highlight-next-line
                disableDefaultUI: true,
                // ...
            },
            // ...
        }),
        // ...
    ]
});
```
</TabItem>
</FrontendPreBuiltUITabs>

If you navigate to `/auth`, you should not see the widget anymore.

## Step 2: Render the component yourself

### If you want to redirect after login 

In this example, after a successful login, the user is automatically redirected to the return value of `getRedirectionURL` (`/` by default). No explicit session existence check is required before rendering the `SignInAndUp` component, as the redirection happens automatically if a session exists.

<FrontendPreBuiltUITabs>
<TabItem value="reactjs">
<Question
    question="Do you use react-router-dom?">
<Answer title="Yes">

<RRDVersionSubTabs>

<TabItem value="v6">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";
import { useNavigate } from "react-router-dom";


SuperTokens.init({
    // ...
    recipeList: [
        ThirdPartyEmailPassword.init({
            signInAndUpFeature: {
                disableDefaultUI: true,
                // ...
            },
            // highlight-start
            async getRedirectionURL(context) {
                if (context.action === "SUCCESS") {
                    return "/dashboard"; // defaults to "/"
                };
            },
            // highlight-end
            // ...
        }),
        // ...
    ]
});

function MyLandingPage() {
    const navigate = useNavigate();
    return (
        <div>
            <Header />
            // highlight-next-line
            <SignInAndUp navigate={navigate} />
            <Footer />
        </div>
    );
}
```

</TabItem>

<TabItem value="v5">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";
import { useHistory } from "react-router-dom5";

SuperTokens.init({
    // ...
    recipeList: [
        ThirdPartyEmailPassword.init({
            signInAndUpFeature: {
                disableDefaultUI: true,
                // ...
            },
            // highlight-start
            async getRedirectionURL(context) {
                if (context.action === "SUCCESS") {
                    return "/dashboard"; // defaults to "/"
                };
            },
            // highlight-end
            // ...
        }),
        // ...
    ]
});

function MyLandingPage() {
    const history = useHistory();
    return (
        <div>
            <Header />
            // highlight-next-line
            <SignInAndUp navigate={history} />
            <Footer />
        </div>
    );
}
```

</TabItem>

</RRDVersionSubTabs>

</Answer>
<Answer title="No">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";

SuperTokens.init({
    // ...
    recipeList: [
        ThirdPartyEmailPassword.init({
            signInAndUpFeature: {
                disableDefaultUI: true,
                // ...
            },
            // highlight-start
            async getRedirectionURL(context) {
                if (context.action === "SUCCESS") {
                    return "/dashboard"; // defaults to "/"
                };
            },
            // highlight-end
            // ...
        }),
        // ...
    ]
});

function MyLandingPage() {
    const navigate = useNavigate();
    return (
        <div>
            <Header />
            // highlight-next-line
            <SignInAndUp />
            <Footer />
        </div>
    )
}
```
</Answer>
</Question>
</TabItem>
</FrontendPreBuiltUITabs>

### If you want to conditionally render the Auth Widget

In the following example, we are conditionally rendering the login form only if the user is not logged in.

<FrontendPreBuiltUITabs>
<TabItem value="reactjs">
<Question
    question="Do you use react-router-dom?">
<Answer title="Yes">

<RRDVersionSubTabs>

<TabItem value="v6">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
import Session from "supertokens-auth-react/recipe/session";
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";
import { useNavigate } from "react-router-dom";

function MyLandingPage() {
    let sessionContext = Session.useSessionContext();
    const navigate = useNavigate();
    if (sessionContext.loading) {
        return null;
    }
    if (sessionContext.doesSessionExist) {
        // We wrap this with <SessionAuth /> so that
        // all claims are validated before showing the logged in UI.
        // For example, if email verification is switched on, and
        // the user's email is not verified, then <SessionAuth />
        // will redirect to the email verification page.
        return (
            <Session.SessionAuth>
                <Header />
                    You are logged in!
                <Footer />
            </Session.SessionAuth>
        )
    } else {
        return (
            <div>
                <Header />
                // highlight-next-line
                <SignInAndUp navigate={navigate} />
                <Footer />
            </div>
        )
    }
}
```

</TabItem>

<TabItem value="v5">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
import Session from "supertokens-auth-react/recipe/session";
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";
import { useHistory } from "react-router-dom5";

function MyLandingPage() {
    let sessionContext = Session.useSessionContext();
    const history = useHistory();
    if (sessionContext.loading) {
        return null;
    }
    if (sessionContext.doesSessionExist) {
        // We wrap this with <SessionAuth /> so that
        // all claims are validated before showing the logged in UI.
        // For example, if email verification is switched on, and
        // the user's email is not verified, then <SessionAuth />
        // will redirect to the email verification page.
        return (
            <Session.SessionAuth>
                <Header />
                    You are logged in!
                <Footer />
            </Session.SessionAuth>
        )
    } else {
        return (
            <div>
                <Header />
                // highlight-next-line
                <SignInAndUp navigate={history} />
                <Footer />
            </div>
        )
    }
}
```

</TabItem>

</RRDVersionSubTabs>

</Answer>
<Answer title="No">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
import Session from "supertokens-auth-react/recipe/session";
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";

function MyLandingPage() {
    let sessionContext = Session.useSessionContext();
    if (sessionContext.loading) {
        return null;
    }
    if (sessionContext.doesSessionExist) {
        // We wrap this with <SessionAuth /> so that
        // all claims are validated before showing the logged in UI.
        // For example, if email verification is switched on, and
        // the user's email is not verified, then <SessionAuth />
        // will redirect to the email verification page.
        return (
            <Session.SessionAuth>
                <Header />
                    You are logged in!
                <Footer />
            </Session.SessionAuth>
        )
    } else {
        return (
            <div>
                <Header />
                // highlight-next-line
                <SignInAndUp />
                <Footer />
            </div>
        )
    }
}
```
</Answer>
</Question>
</TabItem>
</FrontendPreBuiltUITabs>

:::important
In the above case, the `SignInAndUp` component gets unmounted immediately upon user login. Consequently, SuperTokens cannot invoke `getRedirectionURL` (if configured) in this situation. Refer to the next case if you need conditional rendering along with redirection.
:::

### If you want to conditionally render the Auth Widget with redirection

In the following example, user will be redirected to the return value of `getRedirectionURL` (`/` by default) after a successful login. However, if a session already exists then user will see the logged in UI without redirection.

<FrontendPreBuiltUITabs>
<TabItem value="reactjs">
<Question
    question="Do you use react-router-dom?">
<Answer title="Yes">

<RRDVersionSubTabs>

<TabItem value="v6">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
import Session from "supertokens-auth-react/recipe/session";
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";
import { useNavigate } from "react-router-dom";

SuperTokens.init({
    // ...
    recipeList: [
        ThirdPartyEmailPassword.init({
            signInAndUpFeature: {
                disableDefaultUI: true,
                // ...
            },
            // highlight-start
            async getRedirectionURL(context) {
                if (context.action === "SUCCESS") {
                    return "/onboarding"; // defaults to "/"
                };
            },
            // highlight-end
            // ...
        }),
        // ...
    ]
});

function MyLandingPage() {
    let sessionContext = Session.useSessionContext();
    const [doesSessionExistOnMount, setDoesSessionExistOnMount] = React.useState<boolean | null>(null);
    const navigate = useNavigate();

    React.useEffect(() => {
        if (sessionContext.loading || doesSessionExistOnMount !== null) {
            return;
        }
        setDoesSessionExistOnMount(!!sessionContext.userId);
    }, [doesSessionExistOnMount, sessionContext]);

    if (sessionContext.loading) {
        return null;
    }

    if (
        !session.userId ||
        doesSessionExistOnMount === null ||
        doesSessionExistOnMount === false
    ) {
        return (
           <div>
                <Header />
                // highlight-next-line
                <SignInAndUp navigate={navigate} />
                <Footer />
            </div>
        );
    }

    // We wrap this with <SessionAuth /> so that
    // all claims are validated before showing the logged in UI.
    // For example, if email verification is switched on, and
    // the user's email is not verified, then <SessionAuth />
    // will redirect to the email verification page.
    return (
        <Session.SessionAuth>
            <Header />
                You are logged in!
            <Footer />
        </Session.SessionAuth>
    );
}
```

</TabItem>

<TabItem value="v5">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
import Session from "supertokens-auth-react/recipe/session";
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";
import { useHistory } from "react-router-dom5";

SuperTokens.init({
    // ...
    recipeList: [
        ThirdPartyEmailPassword.init({
            signInAndUpFeature: {
                disableDefaultUI: true,
                // ...
            },
            // highlight-start
            async getRedirectionURL(context) {
                if (context.action === "SUCCESS") {
                    return "/onboarding"; // defaults to "/"
                };
            },
            // highlight-end
            // ...
        }),
        // ...
    ]
});

function MyLandingPage() {
    let sessionContext = Session.useSessionContext();
    const [doesSessionExistOnMount, setDoesSessionExistOnMount] = React.useState<boolean | null>(null);
    const history = useHistory();

    React.useEffect(() => {
        if (sessionContext.loading || doesSessionExistOnMount !== null) {
            return;
        }
        setDoesSessionExistOnMount(!!sessionContext.userId);
    }, [doesSessionExistOnMount, sessionContext]);

    if (sessionContext.loading) {
        return null;
    }

    if (
        !session.userId ||
        doesSessionExistOnMount === null ||
        doesSessionExistOnMount === false
    ) {
        return (
           <div>
                <Header />
                // highlight-next-line
                <SignInAndUp navigate={history} />
                <Footer />
            </div>
        );
    }

    // We wrap this with <SessionAuth /> so that
    // all claims are validated before showing the logged in UI.
    // For example, if email verification is switched on, and
    // the user's email is not verified, then <SessionAuth />
    // will redirect to the email verification page.
    return (
        <Session.SessionAuth>
            <Header />
                You are logged in!
            <Footer />
        </Session.SessionAuth>
    );
}
```

</TabItem>

</RRDVersionSubTabs>

</Answer>
<Answer title="No">

```tsx
import React from "react";
// highlight-start
import { SignInAndUp } from 'supertokens-auth-react/recipe/thirdpartyemailpassword/prebuiltui';
import Session from "supertokens-auth-react/recipe/session";
// highlight-end
// @ts-ignore
import Header from "./header";
// @ts-ignore
import Footer from "./footer";

SuperTokens.init({
    // ...
    recipeList: [
        ThirdPartyEmailPassword.init({
            signInAndUpFeature: {
                disableDefaultUI: true,
                // ...
            },
            // highlight-start
            async getRedirectionURL(context) {
                if (context.action === "SUCCESS") {
                    return "/onboarding"; // defaults to "/"
                };
            },
            // highlight-end
            // ...
        }),
        // ...
    ]
});

function MyLandingPage() {
    let sessionContext = Session.useSessionContext();
    if (sessionContext.loading) {
        return null;
    }
    if (sessionContext.doesSessionExist) {
        // We wrap this with <SessionAuth /> so that
        // all claims are validated before showing the logged in UI.
        // For example, if email verification is switched on, and
        // the user's email is not verified, then <SessionAuth />
        // will redirect to the email verification page.
        return (
            <Session.SessionAuth>
                <Header />
                    You are logged in!
                <Footer />
            </Session.SessionAuth>
        )
    } else {
        return (
            <div>
                <Header />
                // highlight-next-line
                <SignInAndUp />
                <Footer />
            </div>
        )
    }
}
```
</Answer>
</Question>
</TabItem>
</FrontendPreBuiltUITabs>

## Step 3: Changing the website path for the login UI

The default path for this is component is `/{websiteBasePath}/`.

If you are displaying this at some custom path, then you need add additional config on frontend:

<FrontendPreBuiltUITabs>
<TabItem value="reactjs">

```tsx
import SuperTokens from "supertokens-auth-react";
import ThirdPartyEmailPassword from "supertokens-auth-react/recipe/thirdpartyemailpassword";

SuperTokens.init({
    appInfo: {
        apiDomain: "...",
        appName: "...",
        websiteDomain: "...",
    },
    recipeList: [
        ThirdPartyEmailPassword.init({
            // ...
        }),
    ],
    //highlight-start
    // The user will be taken to the custom path when then need to login.
    getRedirectionURL: async (context) => {
        if (context.action === "TO_AUTH") {
            return "/custom-login-path";
        };
    }
    //highlight-end
})
```
</TabItem>
</FrontendPreBuiltUITabs>

## Optional: Prevent redirection after SignIn / Up

Upon successful login, SuperTokens redirects the user to the return value of `getRedirectionURL` provided in the recipe config. By default or when returning `undefined` from `getRedirectionURL`, the user is redirected to the `/` route. If you wish to completely prevent redirection, return `null` instead. Typically, this is used if you want to embed the sign in / up component in a popup and want to just dismiss the popup post login.

<FrontendPreBuiltUITabs>
<TabItem value="reactjs">

```tsx
import SuperTokens from "supertokens-auth-react";
import ThirdPartyEmailPassword from "supertokens-auth-react/recipe/thirdpartyemailpassword";

SuperTokens.init({
    appInfo: {
        apiDomain: "...",
        appName: "...",
        websiteDomain: "..."
    },
    recipeList: [
        ThirdPartyEmailPassword.init({
            //highlight-start
            getRedirectionURL: async (context) => {
                // Prevent redirection after successful sign in/up by returning null.
                if (context.action === "SUCCESS") {
                    return null;
                };
            },
            //highlight-end
        })
    ],
})
```
</TabItem>
</FrontendPreBuiltUITabs>

:::note
To avoid redirection for failed claims, such as Email Verification, reach out to us on [Discord](https://supertokens.com/discord) for assistance.
:::

## Optional: Rendering Auth components together in a popup and page

You may choose to display the Auth UI in a full page and in a popup in the same app. For the full-page Auth UI, redirection after a successful login is preferred, whereas in the popup, it is not. In this scenario, you have the option to check either the current URL or the `userContext` object in `getRedirectionURL` to decide whether to return null or not.

</PreBuiltUIContent>

<CustomUIContent>
This guide is not applicable for Custom UI.
</CustomUIContent>

</PreBuiltOrCustomUISwitcher>