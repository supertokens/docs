...
- This can happen during association of a third party login to an existing session's account.
    ...
    - Output JSON:
        ...
        ```
- In order to link the third party user with the session user, we need to make sure that the session user is a primary user. However, that can fail if there exists another primary user with the same email, and in this case, this error is returned to the frontend.
