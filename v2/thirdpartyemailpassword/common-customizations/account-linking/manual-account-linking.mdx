...
- `AccountLinking.isEmailChangeAllowed`: Given the recipe user id and the new email for update, this function returns `true` if it's safe to update the email, else `false`. TODO..