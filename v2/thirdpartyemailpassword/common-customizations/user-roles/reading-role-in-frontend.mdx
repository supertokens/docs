...
:::tip
You can pass `requireAuth={false}` to `ThirdPartyEmailPasswordAuth` if you want the `<Dashboard />` component to load even if there is no session. In this case, you would need to check if a session exists in the `<Dashboard />` component.
:::
