...
from supertokens_python.recipe.emailpassword.asyncio import get_user_by_email as get_user_by_email_emailpassword
...
async def some_func():
    ...
    email_password_user_info = await get_user_by_email_emailpassword("public", "test@example.com")
...
async def get_user_info_api(session: SessionContainer = Depends(verify_session())):
    ...
    if thirdparty_user is None:
        emailpassword_user = await get_user_by_id_emailpassword(user_id)
        if emailpassword_user is not None:
            print(emailpassword_user)
    else:
...
def get_user_info_api():
    ...
    if thirdparty_user is None:
        emailpassword_user = get_user_by_id_emailpassword(user_id)
        if emailpassword_user is not None:
            print(emailpassword_user)
    else:
        print(thirdparty_user)
