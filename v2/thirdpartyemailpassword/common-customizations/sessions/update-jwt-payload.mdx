---
id: update-jwt-payload
title: Update JWT Payload
hide_title: true
---

<!-- COPY DOCS -->
<!-- ./session/common-customizations/sessions/update-jwt-payload.mdx -->

import BackendSDKTabs from "/src/components/tabs/BackendSDKTabs"
import NodeJSFrameworkSubTabs from "/src/components/tabs/NodeJSFrameworkSubTabs";
import PythonFrameworkSubTabs from "/src/components/tabs/PythonFrameworkSubTabs";
import PythonSyncAsyncSubTabs from "/src/components/tabs/PythonSyncAsyncSubTabs";
import BackendSDKCasing from "/src/components/BackendSDKCasing";
import TabItem from '@theme/TabItem';


# Update JWT Payload


## Method 1) After session verification

<BackendSDKTabs>
<TabItem value="nodejs">
<NodeJSFrameworkSubTabs>
<TabItem value="express">

```jsx
let { verifySession } = require("supertokens-node/recipe/session/framework/express");

app.post("/updateinfo", verifySession(), async (req, res) => {

    let session = req.session;

    let currJWTPayload = session.getJWTPayload();

    // highlight-start
    await session.updateJWTPayload(
        {newKey: "newValue", ...currJWTPayload}
    );
    // highlight-end

    res.json({message: "successfully updated JWT payload"})
});
```
</TabItem>
<TabItem value="hapi">

```jsx
let { verifySession } = require("supertokens-node/recipe/session/framework/hapi");

server.route({
    path: "/updateinfo",
    method: "post",
    options: {
        pre: [
            {
            method: verifySession()
            },
        ],
    },
    handler: async (req, res) => {
        let session = req.session;

        let currJWTPayload = session.getJWTPayload();

        // highlight-start
        await session.updateJWTPayload(
                {newKey: "newValue", ...currJWTPayload}
        );
        // highlight-end
        return res.response({message: "successfully updated JWT payload"}).code(200);
    }
})
```
</TabItem>
<TabItem value="fastify">

```jsx
let { verifySession } = require("supertokens-node/recipe/session/framework/fastify");

fastify.post("/updateinfo", {
    preHandler: verifySession(),
}, (req, res) => {
    let session = req.session;

    let currJWTPayload = session.getJWTPayload();

    // highlight-start
    await session.updateJWTPayload(
        {newKey: "newValue", ...currJWTPayload}
    );
    // highlight-end
    res.send({message: "successfully updated JWT payload"});
});
```

</TabItem>
<TabItem value="awsLambda">

```jsx
let { verifySession } = require("supertokens-node/recipe/session/framework/awsLambda");

async function updateinfo(awsEvent, _) => {
    let session = awsEvent.session;

    let currJWTPayload = session.getJWTPayload();

    // highlight-start
    await session.updateJWTPayload(
        {newKey: "newValue", ...currJWTPayload}
    );
    // highlight-end

    return {
        body: JSON.stringify({message: "successfully updated JWT payload"}),
        statusCode: 200,
    };
});

exports.handler = verifySession(updateinfo);

```

</TabItem>
<TabItem value="koa">

```jsx
let { verifySession } = require("supertokens-node/recipe/session/framework/koa");

router.post("/updateinfo", verifySession(), (ctx, next) => {
    let session = ctx.session;

    let currJWTPayload = session.getJWTPayload();

    // highlight-start
    await session.updateJWTPayload(
        {newKey: "newValue", ...currJWTPayload}
    );
    // highlight-end
    ctx.body = {message: "successfully updated JWT payload"};
});
```

</TabItem>
<TabItem value="loopback">

```jsx
import { verifySession } from "supertokens-node/recipe/session/framework/loopback";
import Session from "supertokens-node/recipe/session";

class UpdateInfo {
    constructor(@inject(RestBindings.Http.CONTEXT) private ctx: MiddlewareContext) {}
    @post("/updateinfo")
    @intercept(verifySession())
    @response(200)
    handler() {
        let session = this.ctx.session;

        let currJWTPayload = session.getJWTPayload();

        // highlight-start
        await session.updateJWTPayload(
            {newKey: "newValue", ...currJWTPayload}
        );
        // highlight-end
        return {message: "successfully updated JWT payload"};
    }
}
```

</TabItem>
<TabItem value="nextjs">

```jsx
import { superTokensNextWrapper } from 'supertokens-node/nextjs'
import { verifySession } from "supertokens-node/recipe/session/framework/express";

export default async function updateInfo(req, res) {
    await superTokensNextWrapper(
        async (next) => {
            await verifySession()(req, res, next);
        },
        req,
        res
    )
    let session = req.session;

    let currJWTPayload = session.getJWTPayload();

    // highlight-start
    await session.updateJWTPayload(
        {newKey: "newValue", ...currJWTPayload}
    );
    // highlight-end
    res.json({message: "successfully updated JWT payload"})
}
```
</TabItem>
<TabItem value="nestjs">

```tsx
@Controller()
export class ExampleController {
  @Post('example')
  @UseGuards(AuthGuard)
  async postExample(@Session() session): Promise<boolean> {
    const currJWTPayload = session.getJWTPayload();

    let currJWTPayload = session.getJWTPayload();

    //highlight-start
    // For more information about "AuthGuard" and the "Session" decorator please read our NestJS guide.
    await session.updateJWTPayload(
        {newKey: "newValue", ...currJWTPayload}
    );
    //highlight-end
    return {message: "successfully updated JWT payload"};
  }
}
```
</TabItem>
</NodeJSFrameworkSubTabs>
</TabItem>
<TabItem value="go">

```go

// We assume that you have wrapped this handler with session.VerifySession
func updateInfo(w http.ResponseWriter, r *http.Request) {
    // retrieve the session object as shown below
	sessionContainer := session.GetSessionFromRequestContext(r.Context())

	currJWTPayload := sessionContainer.GetJWTPayload()

	currJWTPayload["newKey"] = "newValue"

	// highlight-next-line
	err := sessionContainer.UpdateJWTPayload(currJWTPayload)
	if err != nil {
		err = supertokens.ErrorHandler(err, r, w)
		if err != nil {
			// TODO: Send 500 to client
		}
		return
	}
}
```

</TabItem>
<TabItem value="python">

<PythonFrameworkSubTabs>
<TabItem value="fastapi">

```python
from supertokens_python.recipe.session.framework.fastapi import verify_session

@app.post('/update-jwt')
async def update_jwt(session: Session = Depends(verify_session())):
    # retrieve the session object as shown below
    current_jwt_payload = session.get_jwt_payload()

    current_jwt_payload['newKey'] = 'newValue'

    # highlight-next-line
    await session.update_jwt_payload(current_jwt_payload)

    return PlainTextResponse(content='success')

```

</TabItem>
<TabItem value="flask">

```python

from supertokens_python.recipe.session.framework.flask import verify_session

@app.route('/update-jwt', methods=['POST'])
@verify_session()
def update_jwt():
    session = g.supertokens

    # retrieve the session object as shown below
    current_jwt_payload = session.get_jwt_payload()

    current_jwt_payload['newKey'] = 'newValue'

    # highlight-next-line
    session.sync_update_jwt_payload(current_jwt_payload)

    return 'success'

```

</TabItem>
<TabItem value="django">

```python
from supertokens_python.recipe.session.framework.django.asyncio import verify_session

@verify_session()
async def update_jwt(request):
    session = request.supertokens
    # retrieve the session object as shown below
    current_jwt_payload = session.get_jwt_payload()

    current_jwt_payload['newKey'] = 'newValue'

    # highlight-next-line
    await session.update_jwt_payload(current_jwt_payload)
```

</TabItem>
</PythonFrameworkSubTabs>

</TabItem>
</BackendSDKTabs>

- We first require session verification in order to get the session object
- Using that object, we call the <BackendSDKCasing>updateJWTPayload</BackendSDKCasing> with new content. This content completely overrides the existing object, that's why we first get the `currJWTPayload` info.
- The result is that the access token is updated in the user's browser cookies. The change is instantly visible on the frontend and the subsequent backend API calls.


## Method 2) Without session verification

:::caution
Changes to the access token payload via this method are reflected in the session only once the session is refreshed. So use method (1) whenever possible.
:::

<BackendSDKTabs>
<TabItem value="nodejs">

```jsx
let Session = require("supertokens-node/recipe/session");

// we first get all the sessionHandles (string[]) for a user
let sessionHandles = Session.getAllSessionHandlesForUser(userId);

// we update all the session's JWT payloads for this user
sessionHandles.forEach(async (handle) => {
      let currJWTPayload = (await Session.getSessionInformation(handle)).jwtPayload;

      //highlight-start
      await Session.updateJWTPayload(handle, 
            {newKey: "newValue", ...currJWTPayload}
      );
      //highlight-end
})
```
</TabItem>
<TabItem value="go">

```go
// we first get all the sessionHandles (string[]) for a user
sessionHandles, err := session.GetAllSessionHandlesForUser("userId")
if err != nil {
    // TODO: handle error
    return
}

// we update all the session's JWT payloads for this user
for _, handle := range sessionHandles {
    sessionInfo, err := session.GetSessionInformation(handle)
    if err != nil {
        // TODO: handle error
        return
    }

    currJWTPayload := sessionInfo.JwtPayload
    currJWTPayload["newKey"] = "newValue"

    //highlight-next-line
    err = session.UpdateJWTPayload(handle, currJWTPayload)
    if err != nil {
        // TODO: handle error
        return
    }
}
```

</TabItem>
<TabItem value="python">

<PythonSyncAsyncSubTabs>

<TabItem value="asyncio">

```python
from supertokens_python.recipe.session.asyncio import get_all_session_handles_for_user, update_jwt_payload, get_session_information

# we first get all the session_handles (List[string]) for a user
session_handles = await get_all_session_handles_for_user("userId")

for handle in session_handles:
    current_jwt_payload = await get_session_information(handle)

    current_jwt_payload = current_jwt_payload['JwtPayload']
    current_jwt_payload["newKey"] = "newValue"
    await update_jwt_payload(handle, current_jwt_payload)
```

</TabItem>
<TabItem value="syncio">

```python
from supertokens_python.recipe.session.syncio import get_all_session_handles_for_user, update_jwt_payload, get_session_information

# we first get all the session_handles (List[string]) for a user
session_handles = get_all_session_handles_for_user("userId")

for handle in session_handles:
    current_jwt_payload = get_session_information(handle)

    current_jwt_payload = current_jwt_payload['JwtPayload']
    current_jwt_payload["newKey"] = "newValue"
    update_jwt_payload(handle, current_jwt_payload)
```

</TabItem>
</PythonSyncAsyncSubTabs>

</TabItem>
</BackendSDKTabs>
