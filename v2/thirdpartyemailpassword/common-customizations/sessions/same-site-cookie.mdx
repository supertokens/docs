...
- ``none``
  ...
  - On Safari however, if third party cookies are blocked (which is the default behaviour), and the website and api domains do not share the same top level domain, then cookies won't go. Please see [this GitHub issue](https://github.com/supertokens/supertokens-core/issues/280) to know about workarounds.
- ``lax``
