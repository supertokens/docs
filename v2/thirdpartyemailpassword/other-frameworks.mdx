...
<!-- COPY DOCS -->
<!-- ./thirdpartyemailpassword/other-frameworks.mdx -->
...
- The session token is a JWT and be [verified](./common-customizations/sessions/with-jwt/jwt-verification)  by the PHP server.
- If the session token is missing or has expired, the PHP server should return a `401` response. This will prompt the SuperTokens Frontend SDK to trigger an automatic refresh flow to generate new session tokens and retry the request. 
