...
You can read the roles assigned to a user in any API after session verification by using `getClaimValue`. Alternatively, check the [Managing roles and users](./managing-roles-and-users) to see how to fetch directly from the DB using the `userId`
