...
To migrate users with social accounts we can simply call the `signInUp` API with the provider Id and the users third party userId.

For example:

If we were importing a user with Google as their provider with their thirdParty userId being `106347997792363870000`, we can run the following curl command to import the user.  
