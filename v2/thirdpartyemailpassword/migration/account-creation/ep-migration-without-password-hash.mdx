...
We modify the `signUp` function to first check if the user signing up has an account with the external provider. If they do we return a `EMAIL_ALREADY_EXISTS_ERROR`
...
def override_emailpassword_functions(original_implementation: RecipeInterface):
    ...
    async def sign_in(email: str, password: str, tenant_id: str, user_context: Dict[str, Any]) -> Union[SignInOkResult, SignInWrongCredentialsError]:
        ...
        if emailpassword_user is None:
            ...
            response = await sign_up(email, password, tenant_id, user_context)
            if isinstance(response, SignUpEmailAlreadyExistsError):
            ...
            return SignInOkResult(response.user)
					...
					Functions: func(originalImplementation epmodels.RecipeInterface) epmodels.RecipeInterface {
						...
						(*originalImplementation.SignIn) = func(email, password, tenantId string, userContext supertokens.UserContext) (epmodels.SignInResponse, error) {
							...
							if emailPasswordUser == nil {
								...
								return epmodels.SignInResponse{
									OK: &struct{ User epmodels.User }{response.OK.User},
								}, nil
    ...
    async def generate_password_reset_token_post(
        ...
        if emailpassword_user is None:
            ...
            if legacy_user_data is not None:
                ...
                response = await sign_up(tenant_id, email, tempPassword, user_context)
                if isinstance(response, SignUpEmailAlreadyExistsError):
                    raise Exception("Should never come here")
    ...
    async def password_reset_post(
        ...
        if (
            ...
            if (
                ...
                await update_user_metadata(
                    response.user_id, {"isUsingTemporaryPassword": None}
                )
    ...
    async def emailpassword_sign_in(
        ...
        if (
            ...
            if legacy_user_info is not None:
                ...
                # Update the user's metadata to remove the isUsingTemporaryPassword flag
                await update_user_metadata(
                    emailpassword_user.user_id, {"isUsingTemporaryPassword": None}
                )
                return SignInOkResult(emailpassword_user)
            return SignInWrongCredentialsError()
        # highlight-end
						...
						(*originalImplementation.SignIn) = func(email, password, tenantId string, userContext supertokens.UserContext) (epmodels.SignInResponse, error) {
							...
							if ok && isUsingTemporaryPassword.(bool) {
								...
								if legacyUserInfo != nil {
									...
									return epmodels.SignInResponse{
										OK: &struct{ User epmodels.User }{*emailPasswordUser},
									}, nil
								}
