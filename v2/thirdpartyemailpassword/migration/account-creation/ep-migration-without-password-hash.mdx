...
from supertokens_python.recipe.thirdpartyemailpassword.interfaces import RecipeInterface, EmailPasswordSignUpOkResult, EmailPasswordSignUpEmailAlreadyExistsError
...
def override_thirdpartyemailpassword_functions(original_implementation: RecipeInterface):
    ...
    async def emailpassword_sign_up(email: str, password: str, user_context: Dict[str, Any]) -> Union[EmailPasswordSignUpOkResult, EmailPasswordSignUpEmailAlreadyExistsError]:
        # check if the user signing in exists in the external provider
        if does_user_exist_in_external_provider(email):
            # Return EmailPasswordSignUpEmailAlreadyExistsError since the user exists in the external provider
            return EmailPasswordSignUpEmailAlreadyExistsError()
        return await original_emailpassword_sign_up(email, password, user_context)

    original_implementation.emailpassword_sign_up = emailpassword_sign_up
    return original_implementation


def does_user_exist_in_external_provider(email: str):
...
from supertokens_python import init, InputAppInfo
from supertokens_python.recipe import thirdpartyemailpassword
from supertokens_python.syncio import create_user_id_mapping
from supertokens_python.recipe.emailverification.syncio import create_email_verification_token, verify_email_using_token
from supertokens_python.recipe.thirdpartyemailpassword.syncio import get_users_by_email, emailpassword_sign_up
...
def override_thirdpartyemailpassword_functions(original_implementation: RecipeInterface):
    ...
    async def emailpassword_sign_in(email: str, password: str, user_context: Dict[str, Any]) -> Union[EmailPasswordSignInOkResult, EmailPasswordSignInWrongCredentialsError]:
        #  Check if an email-password user with the input email exists in SuperTokens
        supertokens_users = get_users_by_email(email, user_context)
        ...
        for user in supertokens_users:
            ...
            if user.third_party_info == None:
                emailpassword_user = user
        
        if emailpassword_user == None:
            # EmailPassword user with the input email does not exist in SuperTokens
            # Check if the input credentials valid in the external provider
            legacy_user_info = validate_and_get_user_info_from_external_provider(email, password)
            if legacy_user_info == None:
                # Credentials are incorrect
                return EmailPasswordSignInWrongCredentialsError()
            
            # Call the sign_up function to create a new SuperTokens user.
            response = emailpassword_sign_up(email, password, user_context)
            if response.user == None:
                raise Exception("Should never come here")
            
            # Map the external provider's userId to the SuperTokens userId
            create_user_id_mapping(response.user.user_id, legacy_user_info.user_id)
            # Set the userId in the response to use the provider's userId
            ...
            if legacy_user_info.isEmailVerified:
                # Generate an email verification token for the user
                generate_email_verification_response = create_email_verification_token(response.user.user_id, email, user_context)
                if generate_email_verification_response.token != None:
                    verify_email_using_token(generate_email_verification_response.token, user_context)
...
def validate_and_get_user_info_from_external_provider(email: str, password: str) -> Union[None, ExternalUserInfo]:
    return None  
...
def override_thirdpartyemailpassword_apis(original_implementation: APIInterface):
    ...
    async def generate_password_reset_token_post(form_fields: List[FormField],
        ...
        if emailpassword_user == None:
            # EmailPassword user with the input email does not exist in SuperTokens
            # Check if the user exists in the legacy provider and retrieve their data
            legacy_user_data = retrieve_user_data_from_external_provider(email)
            if legacy_user_data != None:
                # Create a SuperTokens account for the user with a temporary password
                tempPassword = generate_password()
                response = emailpassword_sign_up(
                ...
                if legacy_user_data.isEmailVerified:
                    # Generate an email verification token for the user
                    generate_email_verification_token_response = create_email_verification_token(
                        response.user.user_id, email, user_context)
                    if generate_email_verification_token_response.token != None:
                        # Verify the user's email
                        verify_email_using_token(
                            generate_email_verification_token_response.token, user_context)

                # We also need to identify that the user is using a temporary password. We do through the userMetadata recipe
                update_user_metadata(legacy_user_data.user_id, {
...
def retrieve_user_data_from_external_provider(email: str) -> Union[None, ExternalUserInfo]:
    # TODO: Retrieve user data if a user with the input email exists in the external provider.
    return None
...
from supertokens_python.types import GeneralErrorResponse
from supertokens_python.recipe.emailpassword.types import FormField
from supertokens_python.recipe.usermetadata.syncio import get_user_metadata, update_user_metadata
...
def override_thirdpartyemailpassword_apis(original_implementation: APIInterface):
    ...
    async def password_reset_post(form_fields: List[FormField],
        ...
        if response.user_id != None:
            ...
            metadata_result = get_user_metadata(response.user_id, user_context)
            if metadata_result.metadata["isUsingTemporaryPassword"] != None:
					...
					APIs: func(originalImplementation tpepmodels.APIInterface) tpepmodels.APIInterface {
						...
						(*originalImplementation.PasswordResetPOST) = func(formFields []epmodels.TypeFormField, token string, options epmodels.APIOptions, userContext supertokens.UserContext) (epmodels.ResetPasswordPOSTResponse, error) {
							...
							if response.OK != nil {
								...
								if metadata["isUsingTemporaryPassword"] != nil {
...
from supertokens_python.recipe.usermetadata.syncio import get_user_metadata, update_user_metadata
from supertokens_python.recipe.emailverification.syncio import create_email_verification_token, verify_email_using_token
from supertokens_python.recipe.thirdpartyemailpassword.syncio import get_users_by_email, emailpassword_sign_up, update_email_or_password
from supertokens_python.recipe.thirdpartyemailpassword.interfaces import RecipeInterface, EmailPasswordSignInOkResult, EmailPasswordSignInWrongCredentialsError
from typing import Dict, Any, Union
...
def override_thirdpartyemailpassword_functions(original_implementation: RecipeInterface):
    ...
    async def emailpassword_sign_in(email: str, password: str, user_context: Dict[str, Any]) -> Union[EmailPasswordSignInOkResult, EmailPasswordSignInWrongCredentialsError]:
        ...
        if emailpassword_user == None:
            # EmailPassword user with the input email does not exist in SuperTokens
            # Check if the input credentials valid in the external provider
            legacy_user_info = validate_and_get_user_info_from_external_provider(
                email, password)
            if legacy_user_info == None:
                # Credentials are incorrect
                return EmailPasswordSignInWrongCredentialsError()
            ...
            # Map the external provider's userId to the SuperTokens userId
            create_user_id_mapping(response.user.user_id,
            ...
            if legacy_user_info.isEmailVerified:
                # Generate an email verification token for the user
                generate_email_verification_response = create_email_verification_token(
                    response.user.user_id, email, user_context)
                if generate_email_verification_response.token != None:
                    verify_email_using_token(
        ...
        # highlight-start
        # Check if the user signing in has a temporary password
        metadata_result = get_user_metadata(emailpassword_user.user_id)
        if metadata_result.metadata["isUsingTemporaryPassword"] != None:
            # Check if the input credentials are valid in the external provider
            legacy_user_info = validate_and_get_user_info_from_external_provider(
                email, password)
            if legacy_user_info != None:
                ...
                # Update the user's metadata to remove the isUsingTemporaryPassword flag
                update_user_metadata(emailpassword_user.user_id, {
                    "isUsingTemporaryPassword": None
                })
