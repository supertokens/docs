        ...
        functions: (originalImplementation) => {
            return {
                ...
                emailPasswordSignIn: async function (input) {
                    ...
                    let superTokensUsers = await ThirdPartyEmailPassword.getUsersByEmail(input.email, input.userContext);
    ...
    override: {
        functions: (originalImplementation) => {
            return {
                ...
                emailPasswordSignIn: async function (input) {
                    // Check if an email-password user with the input email exists in SuperTokens
                    let superTokensUsers = await this.getUsersByEmail({ email: input.email, tenantId: input.tenantId, userContext: input.userContext });
                    let emailPasswordUser = undefined;
        ...
        if "isUsingTemporaryPassword" in metadata_result.metadata and metadata_result.metadata["isUsingTemporaryPassword"] is True:
            ...
            if legacy_user_info is not None:
                ...
                await update_email_or_password(
                    emailpassword_user.user_id, None, password, False, user_context)
                # Update the user's metadata to remove the isUsingTemporaryPassword flag
						...
						(*originalImplementation.EmailPasswordSignIn) = func(email, password string, userContext supertokens.UserContext) (tpepmodels.SignInResponse, error) {
							...
							if ok && isUsingTemporaryPassword.(bool) {
								...
								if legacyUserInfo != nil {
									...
									_, err = thirdpartyemailpassword.UpdateEmailOrPassword(emailPasswordUser.ID, nil, &password, &usePasswordPolicy)
									if err != nil {
										return tpepmodels.SignInResponse{}, err
									}
