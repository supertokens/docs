...
- You will need to a JWT decodeing library to extract the Auth0 userId from the `access_token` on the backend. In our implementation we used [`jsonwebtoken`](https://www.npmjs.com/package/jsonwebtoken) to decode the access token.
- You will need to configure your `Auth0Provider` to store the users `access_token` in localstorage instead of memory so Auth0's `/authorize` endpoint is not called when retrieving the `access_token`. This can be done by setting `cacheLocation` to `localstorage` in `Auth0Provider`'s config.

### Frontend Changes:

- On your frontend, on page load, check if a valid Auth0 session exists. If a valid session exists send a request to the API defined in the next section with the Auth0 access token.
...
- Create a new API on your backend, this will be called to migrate your Auth0 session:
