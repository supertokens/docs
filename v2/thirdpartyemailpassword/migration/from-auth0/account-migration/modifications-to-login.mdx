            ...
            return {
                ...
                signIn: async function (input) {
                    ...
                    if(await doesUserExistInAuth0(input.email)){
                        ...
                        // If a user with the input email exists we want to check if they are an Email-Password account.
                        ...
                        // Box 5: Set the userId in the response to use the Auth0 userId
...
let validateAndGetUserInfoFromAuth0 = async (email, password) => {
    ...
    try {
        ...
        // retive the users info using the access token if valid credentials were passed
        // TODO: add your Auth0 domain 
        ...
        functions: (originalImpl) => {
            return {
                ...
                signInUp: async function (input) {
                    ...
                    // Box 2: get the user info from Auth0 with the Social Provider id
                    let auth0UserInfo = await getThirdPartyUserFromAuth0(input.user.thirdParty.userId);

                    // Box 3: does social account exist in Auth0
                    if (auth0UserInfo !== undefined) {
                        ...
                        if (response.createdNewUser) {
                            // Box 6: map the Auth0 userId to the SuperTokens userId, to learn more about the user mapping please check the User Id  Mapping section.
                            // If you have not stored the users Auth0 userId, you can ignore this step
                            setUserIdMapping({ auth0UserId: auth0UserInfo.user_id, supertokensUserId: response.user.id })

                            // Box 7: Set the newly created flag value to false in the response
                            response.createdNewUser = false
                        }

                        // Box 8: Set the userId in the response to use the Auth0 userId.
                        // If you have not stored the users Auth0 userId, you can ignore this step
...
There are two methods of validating user creden:
- Using Auth0's APIs
- Using the exported user data and password hashes from Auth0
