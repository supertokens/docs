...
To get started, you will first need to setup SuperTokens on your frontend and backend. You can do so by following the [quick setup](/docs/thirdpartyemailpassword/quick-setup/video-tutorial)

:::info
The following code is implemented for React and Nodejs, but, you can apply the same logic in your own tech stack.
:::
...
The `Step` number mentioned the comments refers to the `Step` number in the flow chart.
