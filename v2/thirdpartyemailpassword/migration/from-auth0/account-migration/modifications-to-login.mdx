...
We will be using Auth0's APIs to validate their credentials. Please refer to Auth0's [documentation](https://auth0.com/docs/api) to get the complete spec for the APIs used below.
...
let doesUserExistInAuth0 = async (email) => {
    ...
    // retrieves users with the input email which are not Social Accounts
...
The Implementation below follows the following sample data structure. Please refer to Auth0's [docs](https://auth0.com/docs/support/manage-subscriptions/export-data) to get the latest information.
