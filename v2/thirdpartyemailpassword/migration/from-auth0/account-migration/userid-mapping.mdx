...
```jsx
// The following function is used in Step 9 of the flow diagram in the Modifications to login section
...
function getAuth0ToSuperTokensUserIdMapping( userId ){
    ...
    let userIdMapping = getUserIdMappingIfExists(allUsersIDMapping, userId)
