...
- If a user has an Auth0 `userId` mapped to an SuperTokens `userId`, then use then use the Supertokens `userId` in the function and set the Auth0  `userId` in the response.
