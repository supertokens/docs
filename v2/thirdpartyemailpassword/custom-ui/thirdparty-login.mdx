...
class AuthActivity : AppCompatActivity() {
    private val googleSignInLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        if ( it.resultCode == Activity.RESULT_OK ) {
            ...
            // TODO: Call SuperTokens sign in API and send the id token
        }
    }
...
In the code snippet above we use the id token flow with Google by using `requestIdToken`, note that we use `requestEmail` because SuperTokens needs atleast the email scope. After the user successfully signs in we get the id token from Google and use it to call the `/signinup` route exposed by the SuperTokens backend SDK which returns the session tokens
