...
class AuthActivity : AppCompatActivity() {
    private val googleSignInLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        if ( it.resultCode == Activity.RESULT_OK ) {
            ...
            // TODO: Call SuperTokens sign in API and send auth code
        }
    }
...
In the code snippet above we configure the Google client to use the auth code flow by calling `requestServerAuthCode` on `GoogleSignInOptions.Builder`, this means that after the user signs in successfully we can retrieve the authorisation code from the result. This code can then be sent to SuperTokens by calling the `/signinup` route exposed by the SuperTokens backend SDK which will return the session tokens
