...
from supertokens_python.recipe.emailpassword.interfaces import ResetPasswordUsingTokenOkResult, PasswordResetPostOkResponse, PasswordResetPostInvalidTokenResponse
...
def override_thirdpartyemailpassword_apis(original_implementation: APIInterface):
    async def password_reset_post(form_fields: List[FormField], token: str,
        ...
        if isinstance(result, ResetPasswordUsingTokenOkResult):
            return PasswordResetPostOkResponse(user_id=result.user_id)
