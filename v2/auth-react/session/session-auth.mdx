...
- Access token payload is updated

`useSessionContext` returns an object:
...
const Dashboard = () => {
  ...
  if (session.loading) {
    ...
    // This will never occur if you set requireAuth to true in the SessionAuth component
    return null;
  }

  const { doesSessionExist, userId, jwtPayload } = session;
