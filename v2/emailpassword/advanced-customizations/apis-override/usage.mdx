...
:::info
See all the [functions that can be overrided here](https://supertokens.io/docs/nodejs/interfaces/recipe_emailpassword.APIInterface.html)
:::
...
- All the APIs you can override can be found [here](https://supertokens.io/docs/nodejs/interfaces/recipe_emailpassword.APIInterface.html), copy the name of the function you would like to override.
                ...
                apis: (originalImplementation) => {
                    return {
                        ...
                        signInPOST: async (input) => {
                            ...
                            let accessTokenPayloadPromise = options.config.sessionFeature.setAccessTokenPayload(user, formFields, "signin");
                            let sessionDataPromise = options.config.sessionFeature.setSessionData(user, formFields, "signin");

                            let accessTokenPayload = await accessTokenPayloadPromise;
                            let sessionData = await sessionDataPromise;

                            await Session.createNewSession(options.res, user.id, accessTokenPayload, sessionData);
                            return {
                                status: "OK",
                                user,
                            };
