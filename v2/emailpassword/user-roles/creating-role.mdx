...
import (
    ...
    "github.com/supertokens/supertokens-golang/recipe/userrolesmodels",
)

func createRole() {
    err := userroles.CreateNewRoleOrAddPermissions("user", []string {
        "read",
    })

    if err != nil {
        if (err.Is(err, userrolesmodels.UnknownRoleError)) {
            // No such role exists
        }
    }
}
