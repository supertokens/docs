...
The login API will return the following headers:
- `st-access-token`: This contains the current access token associated with the session. You should save this in your application (e.g., in frontend cookies using `document.cookie`).
- `st-refresh-token`: This contains the current refresh token associated with the session. You should save this in your application (e.g., in frontend cookies using `document.cookie`).
- `front-token` header: This contains information about the access token:
    - The userID
    - The expiry time of the access token
    - The payload added by you in the access token.
