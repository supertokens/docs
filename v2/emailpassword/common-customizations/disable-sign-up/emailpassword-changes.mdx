...
app.post("/create-user", verifySession({
    ...
    await ^{recipeNameCapitalLetters}.sendResetPasswordEmail(signUpResult.user.id);
...
func createUserAPI(w http.ResponseWriter, r *http.Request) {
	...
	// we successfully created the user. Now we should send them their invite link
	_, err := ^{codeImportRecipeName}.SendResetPasswordEmail(signUpResult.OK.User.ID)
	if err != nil {
		// TODO: send 500 to the client
		return
	}
...
For a multi tenant use case, you can pass in a tenantId when creating a user via the `signUp` function call. This will ensure that the user can only sign into that tenant.
