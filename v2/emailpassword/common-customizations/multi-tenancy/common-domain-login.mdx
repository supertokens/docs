...
If you have [followed the pre built UI setup](../../pre-built-ui/setup/frontend), when you visit the login screen, you should see the email password login UI. We want to change the flow so that the user is first asked to enter their tenant ID and then the login UI is shown based on the tenant ID.
...
For this, we must first disable the default sign in UI and create our own route on which we will built our component. Start by setting the `disableDefaultUI` config in the `supertokens.init` function call as shown below:
...
Now if you visit the login page, you will no longer see the email password component. We will now create our own login route on which we will display the following component:
...
- Finally, after saving the tenantId in localstorage, we make the component render the `SignInAndUp` UI component from SuperTokens lib which will render the UI we had disabled previously.
...
When the user clicks on the email password sign in / sign up button, we call the relevant API along with the user's `tenantId`. For example, here is how we add the `tenantId` to the sign up function call:
...
async function signUpClicked(email: string, password: string, tenantId: string) {
    try {
...
All the steps for mobile app login are similar to the [emailpassword login steps](../../custom-ui/email-password-login), except that when you are calling the sign in / up API, you also need to pass in the tenantId in the request body. An example of this is shown below:
