...
:::info NOTE
You can wrap your entire application in `SuperTokensWrapper`. This will provide session information in every wrapped component just like below, but it will not redirect logged out users to the sign in screen.
:::
...
function Dashboard(props: any) {
    ...
    // doesSessionExist will always be true if this is wrapped in ^{recipeNameCapitalLetters}Auth without requireAuth set to false
    // but it can be used outside of protected components to tell if the user has an active session
