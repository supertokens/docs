...
Once this feature is enabled, each session verification attempt will result in a database call, as a result this may slow down all of your API calls.

For example, the API defined in [Method 1](#method-1-revoking-via-the-req-object) requres session verification, if the access token blacklisting feature is enabled, the api will slow down.
