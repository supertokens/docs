...
- Changing the `refresh_token_validity` value (time in mins): Determines the overall session timeout.
   ...
   - The minimum value is 0.0166667 mins (1 second).
...
- If the session is revoked, and the user somehow still has their access token, they will be able to query the APIs until it expires (unless you switch on [access token blacklisting](./access-token-blacklisting)).
:::
