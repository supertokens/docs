...
func main() {
	// we first get all the sessionHandles (string[]) for a user
	sessionHandles, err := session.GetAllSessionHandlesForUser("userId", nil)
	if err != nil {
		// TODO: handle error
		return
	}
	...
	for _, handle := range sessionHandles {
		sessionInfo, err := session.GetSessionInformation(handle, nil)
		if err != nil {
			// TODO: handle error
			return
		}
		...
		//highlight-next-line
		err = session.UpdateAccessTokenPayload(handle, currAccessTokenPayload, nil)
		if err != nil {
			// TODO: handle error
			return
		}
