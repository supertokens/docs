...
When you add claims to the JWT paylaod ([see here](./custom-claims)), the claims are also added to the access token payload. This makes it possible to read the values without needing to decode the JWT.
