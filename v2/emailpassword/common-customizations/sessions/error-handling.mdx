	...
	supertokens.Init(supertokens.TypeInput{
		RecipeList: []supertokens.Recipe{
			session.Init(&sessmodels.TypeInput{
				//highlight-start
				ErrorHandlers: &sessmodels.ErrorHandlers{
					OnInvalidClaim: func(message string, req *http.Request, res http.ResponseWriter) error {
						// TODO: Write your own logic and then send a 403 response to the frontend
						return nil
					},
				},
...
async def invalid_claim_callback(req: BaseRequest, err: str, response: BaseResponse):
    # TODO: Write your own logic and then send a 403 response to the frontend
    return response
