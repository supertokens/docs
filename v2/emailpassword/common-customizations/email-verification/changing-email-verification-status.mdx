...
async function manuallyVerifyEmail(recipeUserId: supertokensTypes.RecipeUserId) {
    try {
...
async def manually_verify_email(user_id: str):
    try:
        # Create an email verification token for the user
        token_res = await create_email_verification_token("public", user_id)
...
def manually_verify_email(user_id: str):
    try:
        # Create an email verification token for the user
        token_res = create_email_verification_token("public", user_id)
...
```tsx
import EmailVerification from "supertokens-node/recipe/emailverification";
import supertokensType from "supertokens-node/types";

async function manuallyUnverifyEmail(recipeUserId: supertokensType.RecipeUserId) {
    try {
