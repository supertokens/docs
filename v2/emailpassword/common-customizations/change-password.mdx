...
func changePasswordAPI(w http.ResponseWriter, r *http.Request) {
	...
	updateResponse, err := ^{rid}.UpdateEmailOrPassword(userID, &userInfo.Email, &requestBody.NewPassword, sessionContainer.GetTenantId() nil)
	if err != nil {
		// TODO: Handle error
		return
	}
...
def change_password():
    ...
    update_response = update_email_or_password(user_id, password=request.json["newPassword"], session.get_tenant_id())
...
When calling the sign in function to verify the user's credentials, you need to also pass in their tenantId as an argument. You can fetch the user's tenantId using `session.getTenantId()`.

Notice that we pass in the tenantId as an argument to the `updateEmailOrPassword` function. This is because the we want to make sure that the user's new password matches the password policy defined for their tenant (if you have defined different password polocies for different tenants).
