...
- **Step 2) Create a SuperTokens account for users trying to sign in if they have an account with the external provider**
  -  We will modify the signin flow to check if the user signing in has an existing account with the external provider. If they do and the input credentials are valid, we create a SuperTokens user and map the external userId to the SuperTokens userId.
    This will allow SuperTokens functions to refrence the user with the external userId.
- **Step 3) Create a SuperTokens account for users who users who have an account with the external provider but have forgotten their password.**
  - Some users who have an account with the external provider and not with SuperTokens might have forgotten their passwords and will trigger the password reset flow.
   Since SuperTokens requires an account to exist to perform a password reset we will need to modify the password reset flow to check that if the user does not exist in SuperTokens and exists in the external provider, then create a SuperTokens account with a temporary password and then continue the password reset flow.
        ...
        functions: (originalImplementaion) => {
            return {
                ...
                signUp: async function(input){
                    // we need to check if the user signing in exists in the external provider
                    if(await doesUserExistInExternalProvider(input.email)){
                        
                        // we return status "EMAIL_ALREADY_EXISTS_ERROR" since the user exists in the external provider
                        return {
            ...
            return {
                ...
                signIn: async function (input) {
                    ...
                    if (await doesUserExistInExternalProvider(input.email)) {
                        ...
                        // we need to check if the user signing in has a temporary password
                        ...
                        if(userMetadata.status === "OK" && userMetadata.metadata.isUsingTemporaryPassword){
                            // we check if the input credentials are valid in the external provider
                            let legacyUserInfo = await validateAndGetUserInfoFromExternalProvider(input.email, input.password);
                            if(legacyUserInfo){
                                // we will update the user's password with the correct password
                                EmailPassword.updateEmailOrPassword({
                                    userId: supertokensUser.id,
                                    password: input.password
                                })

                                // we will also update the user's metadata to remove the isUsingTemporaryPassword flag
...
Additionally another check that needs to be made is that if a user signs in with an email that exists in the external provider and SuperTokens
- In this case we will need to check if the user has the `isUsingTemporaryPassword` flag set in their metadata.
This is flag is set in the password reset flow and it's relevance is described in [Step 3](#step-3-override-the-password-reset-flow).
- If the user has this flag, we check if the input credentials to the `signIn` function are valid in the external provider. 
If they are, we update the users account with the password and remove the `isUsingTemporaryPassword` flag from it's metadata. If the flag is present and the credentials are invalid we throw a `WRONG_CREDENTIALS_ERROR`. 


## Step 3) Create a SuperTokens account for users who users who have an account with the external provider but have forgotten their password.

Some users who do not have a supertokens account but have existing account with the external provider may have forgotten their passwords and will initiate the password reset flow.
We will need to modify SuperTokens reset password API to account for this scenarion since password resets requires an existing SuperTokens account. 
...
The code above overrides the `passwordResetPOST` API to remove the `isUsingTemporaryPassword` metadata if it exists on a successful password reset.
