...
If they are, we update the users account with the password and remove the `isUsingTemporaryPassword` flag from it's metadata. If the flag is present and the credentials are invalid we return a `WRONG_CREDENTIALS_ERROR`. 
