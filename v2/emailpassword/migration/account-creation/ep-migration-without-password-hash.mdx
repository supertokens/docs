...
- **Step 3) Create a SuperTokens account for users who have an account with the external provider but have forgotten their password.**
  - Some users who have an account with the external provider and not with SuperTokens may have forgotten their passwords and will trigger the password reset flow.
   Since SuperTokens requires an existing account to send the reset password email to, we will need to modify the password reset flow to check that if the user needs to be migrated. If they do, we create a SuperTokens account with a temporary password, import their user data and continue the password reset flow.
...
- **Step 5) Update the login flow to account for the `isUsingTemporaryPassword` flag**
  - We also update the login flow to prevent signins from accounts who have the `isUsingTemporaryPassword` flag and have not completed the password reset flow.
...
- Similar to [Step 1](#step-1-override-the-login-flow), we need to determine whether to migrate the user or not.
...
Apart from the changes we made in [Step 1](#step-1-prevent-signups-from-users-who-exist-in-the-external-provider), we also need to account for users who have a initiated password reset but have not completed the flow.
This results in a SuperTokens account being created with a temporary password. There are two cases we need to handle:
...
- If a user initiates a password reset but remembers their password, they should be able to sign in. 
  - We should also account for the scenario where a user initiates a reset password flow, but, remembers their password and tries to signin.
...
- User forgets their passwords and tries the password reset flow:
  - In this situation, the regular password reset flow will not work since password resets require an existing account whose email the password reset link would be sent to..
  - The changes proposed in [Step 3](#step-3-create-a-supertokens-account-for-users-who-have-an-account-with-the-external-provider-but-have-forgotten-their-password) and [Step 4](#step-4-remove-the-isusingtemporarypassword-flag-on-successful-password-reset) resolve the this edge case.  
- User starts the password reset flow but they remember their password and try to login with the valid password.
  - In this scenario if the user starts the password reset flow a SuperTokens account with a temporary password is created. Instead of completing the password reset flow they remember their password and try to sign in.
  In this case the user should be able to successfully sign in and the account should be updated with the valid password. 
  - The changes proposed in [Step 5](#step-5-update-the-login-flow-to-account-for-the-isusingtemporarypassword-flag) allows for this flow.

 
## When can I stop using my legacy auth provider?

Once you finish implementing the changes above you will have to decide when to end the migration period.

Ideally you would like to keep the migration flow in place until all your users have been migrated over but, querying your legacy authentication provider on each login request is not feasible in the long term.

Your migration period could be decided by either of the following factors:
- A time window (2-3 months) within which "just-in-time" migration is possible.
...
- You will then email the users encouraging them to go through the password reset flow.  
