...
- **Step 5) Update the login flow to account for the `isUsingTemporaryPassword` flag**
  - We also update the login flow to disallow signins from accounts who have the `isUsingTemporaryPassword` flag and have not completed the password reset flow. 
...
- We first need to determine if the user needs to be migrated or not by check if an account with the input credentials does not exist in SuperTokens but exists in the external provider.
...
Since password resets require an existing SuperTokens account to send the password reset email to, the password reset API will need to be modified to create a SuperTokens account if the user exists in the external provider. 
...
- Prevent signin for accounts who have temporary passwords
...
- If a user initiates a password reset, but, remembers their password then they should be able to sign in. 
