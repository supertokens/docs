...
To block signups from users with account in the external provider we will override the function that handles signup when initializing the `EmailPassword` recipe on the backend.
...
The first step ensures that signups from users with legacy accounts are blocked. With this change users who have an account with the external provider will be prompted to login if they try to signup.
...
- We also need to account for an additional scenario involving the password reset flow. 
This will be made clear in the next step but essentially there could be a state where the user initiates the password 


## Step 3) Override the password reset flow 
