...
from supertokens_python.recipe.emailpassword.syncio import get_user_by_email, sign_up
...
def override_email_password_functions(original_implementation: RecipeInterface):
    ...
    async def sign_in(email: str, password: str, user_context: Dict[str, Any]) -> Union[SignInOkResult, SignInWrongCredentialsError]:
        # check if the user exists in SuperTokens
        supertokens_user = get_user_by_email(email)
        if supertokens_user == None:
            ...
            # Call the signup function to create a new SuperTokens user
            sign_up_response = sign_up(email, password, user_context)
            if sign_up_response.user == None:
                raise Exception("Should never come here")
            
            # Map the external provider's user_id to the SuperTokens user_id
            create_user_id_mapping(sign_up_response.user.user_id, legacy_user_info.user_id)
            ...
            if legacy_user_info.isEmailVerified:
                generate_email_verification_token_response = create_email_verification_token(sign_up_response.user.user_id, email)
                if generate_email_verification_token_response.token != None:
                    # Verify the user's email
                    verify_email_using_token(generate_email_verification_token_response.token)
...
def override_email_password_apis(original_implementation: APIInterface):
    ...
    async def generate_password_reset_token_post(form_fields: List[FormField],
        ...
        # Check if the user exists in SuperTokens
        supertokens_user = get_user_by_email(email, user_context)
        if supertokens_user == None:
            # EmailPassword user with the input credentials does not exist in SuperTokens
            # Check if the input credentials are valid in the external provider
            legacy_user_info = retrieve_user_data_from_external_provider(email)
            if legacy_user_info != None:
                # Create a SuperTokens account with a temporary password
                temp_password = generate_password()
                response = sign_up(email, temp_password, user_context)
                ...
                # We also need to identify that the user is using a temporary password. We do this through the UserMetadata recipe
                update_user_metadata(legacy_user_info.user_id, {
...
from supertokens_python.recipe import emailpassword
from supertokens_python.recipe.emailpassword.interfaces import RecipeInterface, SignInOkResult, SignInWrongCredentialsError
from supertokens_python.recipe.emailpassword.syncio import get_user_by_email, sign_up, update_email_or_password
from supertokens_python.recipe.emailverification.syncio import create_email_verification_token, verify_email_using_token
...
def override_email_password_functions(original_implementation: RecipeInterface):
    ...
    async def sign_in(email: str, password: str, user_context: Dict[str, Any]) -> Union[SignInOkResult, SignInWrongCredentialsError]:
        ...
        # highlight-start
        # Check if the user signing in has a temporary password
        metadata_result = get_user_metadata(supertokens_user.user_id, user_context)
        if metadata_result.metadata["isUsingTemporaryPassword"] != None:
            # Check if the input credentials are valid in the external provider
            legacy_user_info = validate_and_get_user_info_from_external_provider(email, password)
            ...
            if legacy_user_info != None:
                # Update the user's password with the correct password
                update_email_or_password(supertokens_user.user_id, None, password, user_context)
