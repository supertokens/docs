...
In this section we will be modifying login to enable the migrating of users with Email Password accounts from another provider to SuperTokens.
...
<img alt="Flow of login to migrate users with email-password accounts in Auth0 to SuperTokens" src="/img/thirdpartyemailpassword/ep-account-migration-agnostic.png" />
...
async function doesUserExistInProvider(email: string): Promise<boolean> {
    return true;
}

let validateAndGetUserInfoFromProvider = async (email: string, password: string): Promise<{
    user_id: string,
    email: string,
} | undefined> => {
    // ...
        ...
        functions: (originalImpl) => {
            return {
                ...
                signIn: async function (input) {
                    ...
                    // Box 2: check if the User exists in the provider
                    if (await doesUserExistInAuth0(input.email)) {
                        ...
                        if (supertokensUser === undefined) {
                            ...
                            // Box 5: validate users credentials in provider
                            ...
                            // Box 6: call the signup function to create a new SuperTokens user.
                            ...
                            // Box 7: map the providers userId to the SuperTokens userId, to learn more about user mapping please check the User Id  Mapping section.
                            ...
                            // Box 8: Set the userId in the response to use the providers userId, to learn more about user mapping please check the User Id  Mapping section.
