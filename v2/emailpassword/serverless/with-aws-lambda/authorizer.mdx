...
<!-- COPY DOCS -->
<!-- ./emailpassword/serverless/with-aws-lambda/authorizer.mdx -->
...
from supertokens_python.recipe.session.exceptions import TryRefreshTokenError, InvalidClaimsError
...
def generate_allow(principal_id, resource, context=None):
    return generate_policy(principal_id, "Allow", resource, context)

def generate_deny(principal_id, resource, context=None):
...
def handle_auth(request: Request, full_path: str):
    ...
    try:
        ...
        if isinstance(e, InvalidClaimsError):
            return generate_deny("", method_arn, {"body": e.to_json()})
