...
<!-- COPY DOCS -->
<!-- ./emailpassword/serverless/with-aws-lambda/jwt-authorizer.mdx -->
...
def override_session_functions(oi: SessionRecipeInterface) -> SessionRecipeInterface:
    ...
    def create_new_session(request, user_id, access_token_payload, session_data, user_context) -> SessionContainer:
        # highlight-next-line
        access_token_payload = {**access_token_payload, "aud": "jwtAuthorizers"}
        return oi_create_new_session(request, user_id, access_token_payload, session_data, user_context)
