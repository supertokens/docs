...
- 14) About User facing functions:
    ...
    - b) These should throw errors in case of error states and only return on success operation. For example, a recipe function might return multiple `status` types, and only one of them is `"OK"`. Then the corresponding user facing function should throw an error for all the types except for the `"OK"` one, in which case it should return a value if needed.
    
    The reason for this is not particularly a strong one, but it is that in the session recipe, some of the user facing functions should throw an error which should be caught by our error handler. If we did not throw errors, then we would have to ask the user to check the return type and pass it to our error handler themselves.
