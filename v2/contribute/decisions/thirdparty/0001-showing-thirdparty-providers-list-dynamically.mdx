...
No change in the config. `usesDynamicLoginMethods` is optional & `false` by default.

** Multi Tenant, static config **

Frontend is initialized Multitenancy.init() with a config to get tenantId.
...
### Add `usesDynamicLoginMethods` config to the thirdparty recipe
