...
Since we now allow users to dynamically add thirdparty provider configurations into the core, and also the user might have configured a static list of thirdparty providers, this causes an ambiguity on what should be the final set of providers list should be used by the SDK
