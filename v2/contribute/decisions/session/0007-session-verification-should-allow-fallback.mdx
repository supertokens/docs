...
Depends on default auth mode decision [0003](/decisions/0003-default-to-cookie-based-if-rid-header-otherwise)
...
- Fallback means: check the default (see [here](0003)) first then the other method.
- Make sure that updating the access token updates in the fallback case use the same as the incoming request. 
(e.g: if we read a session from cookies then `updateAccessTokenPayload` should update the cookies, not headers)

### Expected behaviour as a table

| auth-mode header 	| sessionRequired 	| Authorization header 	| Access token cookie 	| Output                    	            |
|------------------	|-----------------	|----------------------	|---------------------	|-----------------------------------------	|
| header           	| true           	| none                 	| *                 	| TRY_REFRESH_TOKEN            	            |
| cookie           	| true           	| *                 	| none                 	| TRY_REFRESH_TOKEN            	            |
| header           	| *             	| exists               	| *                 	| Based on session validation of header 	|
| cookie           	| *             	| *                 	| exists               	| Based on session validation of cookie 	|
| header           	| false           	| none                 	| *                 	| undefined                   	            |
| cookie           	| false           	| *                   	| none                 	| undefined                   	            |
| not set          	| true           	| none                 	| none                 	| TRY_REFRESH_TOKEN            	            |
| not set          	| *             	| exists               	| *                    	| Based on session validation of header 	|
| not set          	| *             	| none               	| exists               	| Based on session validation of cookie 	|
| not set          	| false           	| none                 	| none                 	| undefined                   	            |
