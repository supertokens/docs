...
- [Create a JWT](./jwt-creation) in the API - containing the userID, session handle (optional) and the ID for the current microservice (optional).
- Send the request to the various microservices with the JWT.

Advantage:
- If we have m1 -> m2 -> m3, then adding the userID to JWT makes it easy for passing the context forward.