...
API keys are used to authenticate requests made from the backend SDK to the SuperTokens core. By default, there is no API key required, but in case you add one in the core's config or are using the managed service, then, you will need to provide the API key in your backend SDK code, if not the core will throw a `401` error. 
