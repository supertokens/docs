...
- That being said, we use `RS256` for access token verification and the public / private keys for the access token are present in the `session_access_token_signing_keys` table in the database.
